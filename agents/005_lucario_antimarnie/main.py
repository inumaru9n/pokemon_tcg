...
class LucarioPolicy:
    ...
    def _opponent_is_marnie_deck(self) -> bool:
        # Marnie's Impidimp / Morgrem / Grimmsnarl ex
        return self._opponent_has({646, 647, 648})

    def _matchup_target_bonus(self, pokemon: Pokemon) -> int:
        """Gated tech: extra targeting value that only fires in a detected matchup."""
        if not self._opponent_is_marnie_deck():
            return 0
        # Each Grimmsnarl evolution triggers Punk Up (searches & attaches up to 5 {D}
        # energies), so KOing the pre-evolutions starves their only real attacker.
        if pokemon.id == 646:  # Impidimp
            return 900
        if pokemon.id == 647:  # Morgrem
            return 400
        if pokemon.id == 112 and len(pokemon.energies) >= 1:  # powered Munkidori
            return 500
        if pokemon.id == 860:  # Snorunt (Froslass chips our ability Pokémon)
            return 150
        return 0
