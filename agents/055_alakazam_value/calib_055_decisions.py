...
usage:
  uv run python agents/055_alakazam_value/calib_055_decisions.py collect --dates 2026-07-09 \
      --out /tmp/dev.pkl [-w 2] [--limit 30]
  uv run python agents/055_alakazam_value/calib_055_decisions.py fit --dev /tmp/dev.pkl \
...
def _ensure_worker():
    ...
    for p in (REPO_ROOT, AGENT_DIR):
        if p not in sys.path:
            sys.path.insert(0, p)
    spec = importlib.util.spec_from_file_location("calib055_agent", main_path)
