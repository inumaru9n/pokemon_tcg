...
デッキ: agents/174_alakazam_clean/deck.csv
...
#   ① 識別力     ARCH_DECK 9系統のうち **2系統以下**にしか入らない
#                ＋ 名前が "Basic " で始まるもの（無制限に積み上がる）を除外 → 母集団85枚
#   ② 学習可能性  教師データでの board_mu が **0.02 以上**                 → 12枚を足切り
#   ＝ **69枚**。メタが動いたら gen_meta_cards.py で①を再生成し②を測り直すこと。
#
# ②を外すと死に次元になり他の特徴の学習を薄める（166: +33枚中18枚が死に、**-2.29pt**）。
# 旧73枚には104の手作り52枚から継承した逸脱4枚が混じっていたので除去した:
#   Basic {R}(2) / Basic {F}(6) = 無制限に積み上がる / TR Archer(1217) = どのARCH_DECKにも
#   無い / TR Petrel(1219) = ①違反（3系統＝汎用札。追跡するなら THREAT_CARDS 側の基準で）
META_CARDS = [11, 13, 14, 15, 18, 19, 20, 66, 89, 92, 93, 96, 104, 112, 119, 120, 121, 174, 235, 305, 341, 342, 343, 344, 345, 379, 380, 381, 387, 400, 401, 414, 431, 434, 646, 647, 648, 741, 742, 743, 756, 848, 849, 860, 1071, 1079, 1081, 1119, 1120, 1123, 1129, 1134, 1142, 1147, 1173, 1174, 1175, 1184, 1198, 1213, 1216, 1218, 1220, 1229, 1257, 1259, 1261, 1264, 1266]
...
#   **1137 Tool Scrapper        mu 0.006 / 0.011   不合格 → 除外**（④は満たすが観測されない）
THREAT_CARDS = [140, 1080, 1197]

# 有効な特徴グループ（None なら全部。tools/featgen_core.py の _GRULES 参照）
FEAT_GROUPS = {'megaprize', 'selctx', 'threat2', 'misc', 'base', 'resource', 'oppseen', 'slot_attr'}
...
# **174（教師のデッキ）のキーを保ったまま値だけ 117 のものに差し替える**（EXP-108/120）。
# deck.csv から動的に作ると 0枚のカード（Shaymin 343 / Nighttime Mine 1266）が
# キーごと消え、left_/pool_ 等の特徴キーが縮んでモデル(449次元)と不一致になる。
# その場合 **例外は出ず _NN_ACTIVE=False で無言フォールバック**するので気づけない。
DECK_COUNTS = {5: 3, 13: 1, 19: 4, 66: 3, 140: 1, 305: 4, 343: 0, 741: 4, 742: 4, 743: 4, 1079: 4, 1081: 4, 1086: 4, 1097: 0, 1129: 0, 1152: 4, 1182: 4, 1184: 0, 1197: 4, 1225: 4, 1231: 4, 1266: 0}
...
#   misc       ターン進行
# ==== 削除する列（EXP-122）====
# **完全重複**: 学習データ435,150決定で厳密に同一だった列。各群1本だけ残す。
#   重複列は「その方向の実効学習率を倍にする」装置として働く（2列の重みの差が
#   初期化の差ぶんしか無いことを実測: 0.549 vs 予測0.522、無関係な列は1.397）が、
#   設計として意図したものではないので整理する。残す基準 = カードIDに直結しない
#   一般的な列（後日デッキを変えても意味を保つ）。
# **定数列**: 訓練中ずっと mu=0 / sd=下限1e-3。本番リプレイ1,130戦のうち1.1パーセント
#   （confused 0.9 / poisoned 0.2）で発火し1,000シグマを噴く（EXP-111の744シグマと同じ経路）。
#   情報を1ビットも運ばないので、クリップでなく除去が正解。
_DROP_DUP = ['act_abra', 'act_alakazam', 'act_dmg', 'act_dudunsparce', 'act_dunsparce', 'act_e', 'act_fezandipiti', 'act_fresh', 'act_hp_ratio', 'act_kadabra', 'act_shaymin', 'deckout_me', 'deckout_op', 'dis_13', 'emax_fezandipiti', 'emax_shaymin', 'hand_1266', 'hand_13', 'left_1079', 'left_1081', 'left_1086', 'left_1097', 'left_1129', 'left_1152', 'left_1182', 'left_1184', 'left_1197', 'left_1225', 'left_1231', 'left_1266', 'left_13', 'left_140', 'left_19', 'left_305', 'left_343', 'left_5', 'left_66', 'left_741', 'left_742', 'left_743', 'line_fezandipiti', 'line_shaymin', 'me_dot_ability', 'ms0_beats_opp', 'ms0_weak_vs_opp', 'my_act_prize', 'n_shaymin', 'op_retreat_cost', 'opp_act_dmg', 'opp_act_energy', 'opp_act_maxdmg', 'opp_act_prize', 'opp_best_dmg_next', 'prz_if_my_act_ko', 'prz_if_opp_act_ko']
_DROP_CONST = ['act_cant', 'act_n_tool', 'hand_n_tool', 'me_asleep', 'me_burned', 'me_paralyzed', 'me_poisoned', 'ms0_tool', 'ms1_tool', 'ms2_tool', 'ms3_tool', 'ms4_tool', 'ms5_tool', 'op_asleep', 'op_burned', 'op_confused', 'op_paralyzed', 'opp_cant', 'os0_resist_vs_me', 'os3_resist_vs_me', 'os4_resist_vs_me']
_DROP = frozenset(_DROP_DUP) | frozenset(_DROP_CONST)
