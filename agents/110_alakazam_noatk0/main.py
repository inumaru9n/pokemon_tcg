"""110_alakazam_feat4 — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。
...
model_110.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[110_alakazam_feat4] class_bias.json を読めない: {_e}", file=sys.stderr)

# ---- モデルと特徴のロード（失敗しても落ちない） ----
_NET = None
_FT = None
if os.environ.get("NN_110", "on") != "off":
    try:
        import feat_110 as _ft
        from nn_110 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_110.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_110.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[110_alakazam_feat4] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[110_alakazam_feat4] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
# ---- 無駄攻撃の抑制 ----
# 打点が0の攻撃はターンを渡すだけの完全な無駄手。実測（対Spidops 20試合）で
# 118回この状態から攻撃しており、禁止したときの次善手は
# retreat 16.1% / カードを使う等 48.3% / END 35.6% ＝ **約64%で1手多く行動できる**。
_NOATK0 = os.environ.get("NOATK0", "on") != "off"
_natk = {"seen": 0, "blocked": 0}


def _wasted_attack(state, sel, my, board, keys):
    """打点0（無効化・エネ不足）の攻撃選択肢のindex集合を返す。

    `my_dmg_nullified` は「相手アクティブが効果無効化で守られている」ことを示す
    検証済みのフラグ。`my_best_dmg == 0` は「そもそも打点が出ない」。
    どちらもターンを渡す以外の意味が無い（Alakazam の Powerful Hand は打点のみの技）。
    **ただし攻撃しか選択肢が無い場合は触らない**（不正選択を避ける）。
    """
    try:
        if not _NOATK0:
            return set()
        gi = keys.index
        # **打点が「不明」な技は止めない**。ATTACKS_056 で解決できない技には
        # Dunsparce の Trading Places（自分のベンチと入れ替え）のような
        # 打点0でも有用なものがある（対Marnieで実測8件）。
        if board[gi("my_var_atk")] > 0 and board[gi("my_dmg_nullified")] <= 0:
            return set()
        if board[gi("my_dmg_nullified")] <= 0 and board[gi("my_best_dmg")] > 0:
            return set()
        atk = {i for i, o in enumerate(sel.option) if o.type == _OptionType.ATTACK}
        if not atk or len(atk) >= len(sel.option):
            return set()          # 攻撃以外に選べる手が無いなら抑制しない
        return atk
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[110_alakazam_feat4] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[110_alakazam_feat4] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
        ...
        # **打点0の攻撃は禁止する**（検証済みの事実。方策は特徴を持っているのに使えない）
        if ctx_i == 0:
            _bad = _wasted_attack(state, obs.select, my, board, _FT.FEAT_KEYS)
            if _bad:
                _natk["seen"] += 1
                if max(range(n_real), key=lambda i: sc[i]) in _bad:
                    _natk["blocked"] += 1
                sc = [(s - 1e6 if i in _bad else s) for i, s in enumerate(sc)]
