...
# ロード順: lightgbm+model_062.txt（ローカル高速）→ model_062.npz（純Python、Kaggle用。
# argmax一致100%検証済み）→ どちらも無ければ052フォールバック
_M057 = None
if os.environ.get("F057_ML", "on") != "off":
    try:
        try:
            import feat_062 as _ft057
        except ImportError:
            import importlib.util as _ilu057

            _fp = os.path.join(_AGENT_DIR_057, "feat_062.py")
            _sp = _ilu057.spec_from_file_location("feat_062", _fp)
        ...
        try:
            ...
            _mp = os.path.join(_dir057, "model_062.txt")
            ...
            _z = _np057.load(os.path.join(_dir057, "model_062.npz"))
...
# ---- 骨格C 初期化 ----
import skel_common as _K
import skel_pols as _KP
_K.setup(dict(
    Abra=Abra, Kadabra=Kadabra, Alakazam=Alakazam, Dunsparce=Dunsparce,
    Dudunsparce=Dudunsparce, Fezandipiti_ex=Fezandipiti_ex, Shaymin=Shaymin,
    Boss_Orders=Boss_Orders, Xerosic=Xerosic, Enhanced_Hammer=Enhanced_Hammer,
    Rare_Candy=Rare_Candy, Buddy_Buddy_Poffin=Buddy_Buddy_Poffin, Poke_Pad=Poke_Pad,
    Basic_Psychic_Energy=Basic_Psychic_Energy, Telepath_Psychic_Energy=Telepath_Psychic_Energy,
    Enriching_Energy=Enriching_Energy, Night_Stretcher=Night_Stretcher,
    Sacred_Ash=Sacred_Ash, Dawn=Dawn, Hilda=Hilda, Lanas_Aid=Lanas_Aid,
    Nighttime_Mine=Nighttime_Mine, PSYCHIC_ENERGY_IDS=PSYCHIC_ENERGY_IDS,
    OptionType=OptionType, AreaType=AreaType, SelectContext=SelectContext,
))
_SKEL_POL = _KP.POLS["C"]
