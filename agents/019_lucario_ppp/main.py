            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    if op_data.weakness == EnergyType.FIGHTING:
                        ...
                        damage -= 30
                    # Full Metal Lab reduces actual damage to metal Pokemon by 30 (applies always).
                    if self.stadium_id == 1244 and op_data.energyType == EnergyType.METAL:
                        damage -= 30

                    # I-045/EXP-019: Premium Power Pro (Item, stackable, +30 to the opponent's
                    # ACTIVE per copy) is folded into the KO check ONLY, not the general scoring,
                    # so targeting in non-KO cases is unchanged (no mirror regression). This lets
                    # the plan see the OHKO on Archaludon ex (HP300) and commit Premium Power Pro
                    # to deny the Jumbo Ice Cream heal, without perturbing other matchups.
                    ko_damage = damage
                    if target_index == 0:
                        ko_damage += 30 * min(self.hand_counts[C.PREMIUM_POWER_PRO], 3)
