"""098_spidops_gen — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: THIRD PTCG Club / sig d5834c603a / 07-17〜07-24（3,064試合）。プール083枠のレプリカ。忠実度ゲート=094 vs 本エージェント ≈5.5%（本番 n=220）
...
model_098.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
_NET = None
_FT = None
if os.environ.get("NN_098", "on") != "off":
    try:
        import feat_098 as _ft
        from nn_098 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_098.npz")
        ...
        import traceback
        print(f"[098_spidops_gen] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[098_spidops_gen] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[098_spidops_gen] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
