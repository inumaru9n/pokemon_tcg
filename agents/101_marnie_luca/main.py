"""101_marnie_luca — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: Luca / sig 1ec0f47981 / 07-16〜07-29（4,906試合）。プールのMarnie's Grimmsnarl ex枠（直近シェア61.1%＝プールの過半）。忠実度ゲート=094 vs 本エージェント ≈41.7%（教師156952a871との対戦527戦）
...
model_101.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[101_marnie_luca] class_bias.json を読めない: {_e}", file=sys.stderr)

# ---- モデルと特徴のロード（失敗しても落ちない） ----
_NET = None
_FT = None
if os.environ.get("NN_101", "on") != "off":
    try:
        import feat_101 as _ft
        from nn_101 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_101.npz")
        if os.path.exists(_mp):
            ...
            if _mc is not None and len(_mc):
                ...
                print("[101_marnie_luca] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[101_marnie_luca] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[101_marnie_luca] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[101_marnie_luca] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
