...
def _search_main(obs: Observation, policy: LucarioPolicy) -> list[int] | None:
        ...
        for _ in range(SEARCH_SAMPLES):
            ...
            for idx in candidates:
                ...
                try:
                    ...
                    totals[idx] += -1e8  # illegal/crashing line: strongly avoid
                    counts[idx] += 1
    finally:
        try:
            search_end()
    ...
    evaluated = [i for i in candidates if counts[i] == SEARCH_SAMPLES]
    if len(evaluated) < 2:
        return None
    best = max(evaluated, key=lambda i: totals[i] / counts[i] + heuristic_scores[i] * 1e-7)
    return [best]
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if obs.select.context == SelectContext.MAIN and obs.current.turn >= 2:
        ...
        try:
            choice = _search_main(obs, policy)
        except Exception:
            choice = None
