"""053_alakazam_mcs — 052 + 毎手探索（MCS）レイヤ (I-100 / EXP-053)。

ベース: 052_alakazam_mimic（=045方策 + deck_v3 + EXP-052の6ルール。下部の052ドキュメント参照）。
Yushin 81f1（LBトップ、毎手探索型: 中央1.58s/手・ゲーム合計122.7s/600s）の
アーキテクチャに寄せる第1段: MAINフェーズの行動選択のみを探索対象にする。

- 層構成: lethal探索（all-winゲート、052から無変更）→ 不発時にMCS層 → 不発時greedy。
  探索API不発環境（例外）では自然に052へフォールバック。
- MCS層: MAIN・turn>=2の決定点で、候補（greedyの手 + 行動クラス別スコア上位代表 + END）
  ごとに N本の決定化フルロールアウト（終局まで、葉評価不要）を回し勝率を推定。
  決定化: 自分側=消去法サンプル、相手側=既視カード疑似デッキ（EXP-050パターン）。
  ロールアウト方策: 自分側=052 policy()。相手側=Alakazamミラーなら052 policy()
  （自己モデル）、非ミラーは汎用greedy（最大表示ダメージ+Boss敵対吊り、050系）。
- 錨ゲート（piKL教訓/EXP-023）: 同一決定化サンプルを全候補で共有（ペア比較）し、
  pts[best] >= pts[greedy] + MCS_DELTA かつ 全サンプル完走（EXP-049規律）のときのみ上書き。
  score<0（方策が意図的に抑制した選択肢）は候補に入れない（RULE_A/Bを再審理しない）。
- 時間管理: _search_time_used をlethal+MCS共用で累積、累計>ソフト予算でN半減、
  >ハード予算でMCS停止。MOVE_BUDGETはN×候補数に比例（EXP-049）。
- フラグ: F053_MCS=off で052同一挙動、F053_N/F053_DELTA/F053_CANDS/F053_OPP/F053_BUDGET。
...
def policy(obs: Observation) -> list[int]:
    ...
    global _policy_last_scores
    _policy_last_scores = scores  # raw option scores (MCS層の候補選定用に公開)
...
# ---------------------------------------------------------------------------
# EXP-053 MCS layer: per-move Monte-Carlo search over class-representative
# candidates, full determinized rollouts to terminal (no leaf evaluation),
# paired samples across candidates, anchored override (greedy + MCS_DELTA).

MCS_ON = os.environ.get("F053_MCS", "on") != "off"
MCS_SAMPLES = max(2, int(os.environ.get("F053_N", "8")))
MCS_DELTA = float(os.environ.get("F053_DELTA", "1"))     # win-points (win=1, draw=0.5)
# δスイープ（vs052 400戦 seed42）: δ=2:53.25 / δ=1:56.00 / δ=0.5:49.25 / δ=1+END:38.25
# → δ=1採用（seed7 800戦で再測定して確定、EXP-053）
MCS_CANDS = max(2, int(os.environ.get("F053_CANDS", "4")))  # incl. greedy (END may add +1)
MCS_OPP = os.environ.get("F053_OPP", "auto")             # auto | self | generic
MCS_END = os.environ.get("F053_END", "off") == "on"      # ENDをfire候補に含める
MCS_BASE_BUDGET = float(os.environ.get("F053_BUDGET", "1.5"))  # sec at N=8 x 4 cands
MCS_ROLLOUT_CAP = 600        # search_step calls per rollout (full game to terminal)
MCS_SOFT_BUDGET = 250.0      # cumulative search sec: beyond -> halve N
MCS_HARD_BUDGET = 400.0      # cumulative search sec: beyond -> MCS off (lethal stays to 450)

F053_STATS: Counter = Counter()
F053_LOG: list = []          # (turn, n, cands, pts, done, greedy, fired) for instrumentation
_F053_LOG_CAP = 0            # set >0 by instrumentation scripts
...
def _determinize_with_opp(obs: Observation, mirror: bool = False):
    """Like _determinize, but the opponent's hidden zones are sampled from a
    pseudo-deck reconstructed from their seen cards (EXP-050 pattern).
    ミラー時は自分のデッキリストを事前分布に使う（seenのみだと序盤の相手が
    ...
    pool: list[int] = []
    if mirror:
        # opponent deck prior = our own 60 cards (same-archetype assumption)
        remaining = my_deck_counts - seen
...
def _opp_policy(obs: Observation) -> list[int]:
    """Generic greedy policy for a non-mirror opponent in rollouts (050系).
    Attacks with the highest listed damage, develops board, plays supporters
    (sampled Boss's Orders pulls our bench adversarially), skips items."""
    select = obs.select
    state = obs.current
...
def _opt_class(obs: Observation, o, my_index: int):
    """Coarse action-class key used to deduplicate MCS candidates."""
    t = o.type
    if t == OptionType.ATTACK:
        return ("ATK", o.attackId)
    if t == OptionType.END:
        return ("END",)
    ...
    if t == OptionType.ABILITY:
        card = get_card(obs, o.area, o.index, my_index)
        return ("ABI", card.id if card is not None else -1)
    if t == OptionType.PLAY:
        card = get_card(obs, AreaType.HAND, o.index, my_index)
        return ("PLAY", card.id if card is not None else -1)
    if t == OptionType.EVOLVE:
        card = get_card(obs, o.area, o.index, my_index)
        return ("EVO", card.id if card is not None else -1,
                o.inPlayArea == AreaType.ACTIVE)
    if t == OptionType.ATTACH:
        card = get_card(obs, o.area, o.index, my_index)
        return ("ATT", card.id if card is not None else -1,
                o.inPlayArea == AreaType.ACTIVE)
    return ("O", int(t))


def _opp_is_mirror(obs: Observation) -> bool:
    op = obs.current.players[1 - obs.current.yourIndex]
    return any(p is not None and p.id in ABRA_LINE
               for p in list(op.active) + list(op.bench))


def _full_rollout(state, my_index: int, mirror: bool, deadline: float):
    """Play the determinized game to terminal (both sides policy-driven).
    Returns winner index (0/1), 2 for draw, or None if incomplete."""
    for _ in range(MCS_ROLLOUT_CAP):
        obs2 = state.observation
        r = obs2.current.result
        if r != -1:
            return r
        if time.perf_counter() > deadline:
            return None
        try:
            if obs2.current.yourIndex == my_index or mirror:
                sel = policy(obs2)
            else:
                sel = _opp_policy(obs2)
            if not sel and obs2.select.minCount > 0:
                sel = _fallback_selection(obs2.select)
        except Exception:
            sel = _fallback_selection(obs2.select)
        sid = state.searchId
        state = search_step(sid, sel)
        try:
            search_release(sid)   # keep per-decision memory bounded
        except Exception:
            pass
    return None


def _mcs_search(obs: Observation) -> list[int] | None:
    global _search_time_used
    ...
    state = obs.current
    my_index = state.yourIndex
    if _search_time_used > MCS_HARD_BUDGET:
        return None
    n = MCS_SAMPLES if _search_time_used <= MCS_SOFT_BUDGET else max(3, MCS_SAMPLES // 2)

    snap = _snapshot_globals()
    try:
        greedy = policy(obs)
        scores = list(_policy_last_scores)
    finally:
        _restore_globals(snap)
    if not greedy:
        return None
    ...
    # ---- candidate set: commit decisions only ----
    # 非コミット同士の差は「同一ターン内の順序違い」でしかなく（ロールアウト方策が
    # スキップした行動を同ターン内で拾い直す）、探索はノイズで順序を再抽選するだけ
    # （較正プローブで一致率-1.3pt）。真に分岐するのはコミット行動（ATTACK/RETREAT
    # =ターンを畳む/盤面を回す）vs greedy続行。これはYushinとの乖離の主戦場
    # （attackタイミング）。ENDは fire先として不採用（較正プローブでEND fire 17件中
    # 本人一致0件=温存方向の上書きは全て誤り。ミラー決定化改善後も候補に戻さない）。
    order = sorted(range(len(select.option)), key=lambda i: scores[i], reverse=True)
    commit_types = (OptionType.ATTACK, OptionType.RETREAT, OptionType.END) if MCS_END \
        else (OptionType.ATTACK, OptionType.RETREAT)
    seen_cls = {_opt_class(obs, select.option[g], my_index)}
    cands = [g]
    # greedyがコミットなら「greedy続行側」代表として最上位の非コミットも候補に入れる
    if select.option[g].type in (OptionType.ATTACK, OptionType.RETREAT, OptionType.END):
        for i in order:
            if i != g and scores[i] >= 0 \
                    and select.option[i].type not in (OptionType.ATTACK, OptionType.RETREAT,
                                                      OptionType.END):
                cands.append(i)
                seen_cls.add(_opt_class(obs, select.option[i], my_index))
                break
    for i in order:
        if len(cands) >= MCS_CANDS + 1:
            break
        o = select.option[i]
        if i == g or o.type not in commit_types:
            continue
        if scores[i] < 0:
            continue   # 方策が意図的に抑制したコミット（RETREAT<0等）は再審理しない
        c = _opt_class(obs, o, my_index)
        if c in seen_cls:
            continue
        seen_cls.add(c)
        cands.append(i)
    ...
    F053_STATS["mcs_calls"] += 1
    t0 = time.perf_counter()
    budget = MCS_BASE_BUDGET * (n / 8.0) * (len(cands) / 4.0)
    deadline = t0 + budget
    mirror = _opp_is_mirror(obs) if MCS_OPP == "auto" else (MCS_OPP == "self")

    pts = {i: 0.0 for i in cands}
    done = {i: 0 for i in cands}
    alive = [i for i in cands if i != g]   # candidates still able to fire
    try:
        for s in range(n):
            if not alive:
                break   # no candidate can beat greedy + delta anymore
            if time.perf_counter() > deadline:
                F053_STATS["mcs_deadline"] += 1
                break
            root = search_begin(obs, *_determinize_with_opp(obs, mirror))
            for idx in [g] + alive:
                if time.perf_counter() > deadline:
                    break
                try:
                    _restore_globals(snap)   # ロールアウト間の状態リーク防止（EXP-043）
                    child = search_step(root.searchId, [idx])
                    res = _full_rollout(child, my_index, mirror, deadline)
                except Exception:
                    F053_STATS["mcs_samp_exc"] += 1
                    res = None
                if res is not None:
                    done[idx] += 1
                    pts[idx] += 1.0 if res == my_index else (0.5 if res == 2 else 0.0)
            if done[g] < s + 1:
                alive = []   # greedy incomplete: nothing can be compared -> stop
                break
            # branch-and-bound: drop candidates that cannot reach pts[g]+delta
            # even by winning all their remaining samples (pts[g] only grows)
            alive = [i for i in alive
                     if done[i] == s + 1
                     and pts[i] + (n - done[i]) >= pts[g] + MCS_DELTA]
    finally:
        try:
            search_end()
        except Exception:
        ...
        F053_STATS["mcs_time_ms"] += int(dt * 1000)
        _search_time_used += dt

    fired = None
    if done[g] >= n:
        full = [i for i in cands if i != g and done[i] >= n]
        if full:
            best = max(full, key=lambda i: (pts[i], scores[i]))
            if pts[best] >= pts[g] + MCS_DELTA:
                F053_STATS["mcs_fired"] += 1
                fired = best
    if _F053_LOG_CAP and len(F053_LOG) < _F053_LOG_CAP:
        F053_LOG.append((state.turn, n,
                         [_opt_class(obs, select.option[i], my_index) for i in cands],
                         dict(pts), dict(done), g, fired))
    return None if fired is None else [fired]
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if obs.select.context == SelectContext.MAIN and obs.current.turn >= 2:
        ...
        if MCS_ON:
            try:
                choice = _mcs_search(obs)
            except Exception:
                F053_STATS["mcs_exc"] += 1
                choice = None
            if choice is not None:
                return choice
