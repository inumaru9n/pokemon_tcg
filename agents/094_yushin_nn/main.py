"""094_yushin_nn — AlphaStar型「全選択NN方策」の模倣学習版 (I-109 Phase SL / EXP-094)。

教師: Yushin Ito の sig 156952a871（Alakazam/Dudunsparce型、直近勝率58.1%）。
07-17〜07-25 の9日分リプレイから**全コンテキストの決定**を抽出して学習した LSTM 方策。

設計（AlphaStarの構成要素を本エンジンに合わせたもの）:
...
- **ルール・GBT・lethal探索は一切使わない**（真のAlphaStar型。NN単独で全選択）

model_094.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
_NET = None
_FT = None
if os.environ.get("F094_NN", "on") != "off":
    try:
        import feat_094 as _ft
        from nn_094 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_094.npz")
        if os.path.exists(_mp):
            ...
            # **board側も必ず検証する**（opt側だけ見ていると board 変更を見逃す。穴B-1）
            ...
            # 学習時のboard/logs次元をnpzから読んで突合（FEAT_KEYSはimport時点ではNone
            # なので、モデル側に保存した値を正とする）
        ...
        import traceback
        print(f"[094] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # 新ゲームの開始ターン以下だと検出できず、ワーカーへの試合の割り当てが変わるだけで
    # 結果が変わる（実測200戦中1戦）。
    # 確実な境界マーカーを使う: ゲームは必ず SETUP_ACTIVE_POKEMON(1) を1回だけ通り、
    ...
    # maxCount=100.0% に対し max(1,minCount)=96.3%（実測5,060決定）。
    # minCountだと ctx5 TO_BENCH(0,2,2) でPoffinが1体しか出ず、ctx7(1,3,3)で
    # サーチが1枚になる＝1試合あたり約3回、カード1〜2枚分を捨てていた。
    ...
    # 選択肢が1個以下＝強制手。**学習系列にはこれらが含まれない**（抽出時にnopt<2を除外）ため、
    ...
    # （実測では相手4種×24試合の6,103決定で遭遇0件だが、未知のカード効果で起こりうる）
    _tc = globals().get("_TRAINED_CTX")
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[094] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        # board次元の検証は FEAT_KEYS が確定する初回決定時に行う（穴B-1）。
        # ここを黙って通すと、特徴を変えたのに古い重みのまま「毎決定index0を返す
        # エージェント」になり、errors=0・_NN_ACTIVE=True のまま気付けない。
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[094] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
        ...
        # 教師はctx2 SETUP_BENCHの36.3%で「置かない」を選ぶ。
        n_real = n
