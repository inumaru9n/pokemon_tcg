...
# ロード順: lightgbm+model_057.txt（ローカル高速）→ model_057.npz（純Python、Kaggle用。
# argmax一致100%検証済み）→ どちらも無ければ052フォールバック
_M057 = None
if os.environ.get("F057_ML", "on") != "off":
    try:
        try:
            import feat_057 as _ft057
        except ImportError:
            import importlib.util as _ilu057

            _fp = os.path.join(_AGENT_DIR_057, "feat_057.py")
            _sp = _ilu057.spec_from_file_location("feat_057", _fp)
        ...
        try:
            ...
            _mp = os.path.join(_dir057, "model_057.txt")
            ...
            _z = _np057.load(os.path.join(_dir057, "model_057.npz"))
