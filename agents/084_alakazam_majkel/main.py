"""084_alakazam_majkel — 07-22メタのAlakazam主流個体 sig=156952a871（Majkel型、
n=1576 WR45.7%、コピー拡散済み）の軽量逆設計レプリカ (I-129c / EXP-084)。
用途: プールv4 Alakazam枠（19.9%）の代表（現行045=81f1758c92の交代要員）。

ベース: 051_alakazam_deck の main.py（=045方策 + _search_time_used毎ゲームリセット +
不活性テックカード条件）。差分:
- deck.csv = 156952a871 そのもの（045比: Enhanced Hammer 3→4 / Nighttime Mine 3→2。
  051のdeck_v3比: Xerosic 4→3 / Hammer 3→4）
- 方策差分（07-22リプレイ250戦の行動頻度集計 + knowledge/study/majkel_closeread.md）:
  - SETUP_ACTIVE: Dunsparce > Abra > Fez > Shaymin（本人116/100/18/16、
    両方持ち時 Dunsparce 42:0。045のAbra>Dunと逆）
  - SETUP_BENCH: 全たねを置く（Abra 68/68, Dunsparce 19/19, Fez 15/17, Shaymin 11/15。
    045のFez/Shaymin skipと逆）
  - 対TRのDudunsparce Land Crushテックを無効化（本人のLand Crush使用0回。
    045独自テックはMajkelに無い=対TR Spidops 15.6%の弱さも含めて再現する）
- 主要頻度の照合（本人/250戦): Xerosic 1.00/g(相手手札4以上で発火) / Hammer 0.84/g /
  Mine 0.60/g(相手スタジアム131:無し18) / Boss 1.08/g / 攻撃はPH 1186:SuperPsy 95:
  Teleport 67:CruelArrow 56:LandCrush 0 → 045の既存条件でほぼ整合、上記3点のみ修正
- lethal探索層は残置（プール役として無害、EXP-084注記）
...
Poke_Pad = 1152          # x4
Enhanced_Hammer = 1081   # x4 (Majkel: 045のx3から+1)
Rare_Candy = 1079        # x3
...
Xerosic = 1197           # x3
Lanas_Aid = 1184         # x1
Nighttime_Mine = 1266    # x2 (stadium)
...
Mist_Energy = 11
Rock_Fighting_Energy = 20  # "Prevent all effects" = Mist同型（041系Garchompが採用。EXP-048 F3の045バグ修正を移植）
BLOCKING_ENERGY_IDS = {Mist_Energy, Rock_Fighting_Energy}
...
def policy(obs: Observation) -> list[int]:
    ...
    # ---- Majkel補正（EXP-084 負けトレース精読）: PHのキルが際どい時は手札減トレーナーを
    # 撃たない。ladder本人は330-400HPのGarchomp等を手札温存→PH一撃で落とす（300-460ダメ）が、
    # 045骨格はキル可能手札でもHammer/Mine/Xerosic等を先に消費し60-120HP残しの削り漏れを
    # 起こす（局所負けの主因）。攻撃可能なアタッカーが居てターゲットが現手札で倒せるとき、
    # 「1枚消費するとキルが崩れる」なら純減トレーナーをスキップする。
    can_attack_now = (active_id == Alakazam and active_has_psychic) or (
        use_kadabra_finish and (active_id == Kadabra and active_has_psychic))
    kill_margin_cards = 999
    if kills_now and can_attack_now and target_pokemon is not None:
        spend = target_hammer_needed + (1 if target_use_boss else 0)
        kill_margin_cards = (hand_size - spend) - (target_pokemon.hp + 19) // 20
    protect_hand_for_kill = kills_now and can_attack_now and kill_margin_cards <= 1
    ...
    # Majkel: 対TRのDudunsparce Land Crushテックは持たない（本人Land Crush使用0回）
    ready_bench_dudun = False
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE:
                ...
                # Majkel: Dunsparce > Abra > Fez > Shaymin（本人116/100/18/16、
                # 両方持ち時 Dunsparce 42:0。045のAbra>Dunと逆）
                ...
                # Majkel: 全たねを置く（Abra 68/68, Dunsparce 19/19, Fez 15/17,
                # Shaymin 11/15。045のFez/Shaymin skipと逆）
                if card.id == Abra:
                    score = 200
                elif card.id == Dunsparce:
                    score = 150
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if bench_free <= 1 and score > 0:
                    score -= 5000
                # Majkel補正: キルが際どい時はたねプレイ(-20ダメ)もスキップ
                if protect_hand_for_kill and len(my_field) > 1 and card.id != Fezandipiti_ex:
                    score = -1
                ...
                # Majkel補正: キルが際どい時、手札純減トレーナーはキルを崩すのでスキップ
                # （HammerはPH無効解除に必要な分=target_hammer_neededだけ許可。
                #   Dawn/Hilda(純増)・Poké Pad(純増減0)・need_bossのBossは対象外）
                if protect_hand_for_kill and score > 0:
                    if card.id == Enhanced_Hammer:
                        if target_hammer_needed == 0:
                            score = -1
                    elif card.id in (Buddy_Buddy_Poffin, Rare_Candy, Night_Stretcher,
                                     Sacred_Ash, Xerosic, Lanas_Aid, Nighttime_Mine):
                        score = -1
            ...
            if card.id in PSYCHIC_ENERGY_IDS:
                if protect_hand_for_kill:
                    score = -1  # Majkel補正: エネ手貼りも-20ダメ。キルが際どい時はスキップ
