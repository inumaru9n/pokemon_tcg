...
# ロード順: lightgbm+model_059.txt（ローカル高速）→ model_059.npz（純Python、Kaggle用。
# argmax一致100%検証済み）→ どちらも無ければ052フォールバック
_M057 = None
if os.environ.get("F057_ML", "on") != "off":
    try:
        try:
            import feat_059 as _ft057
        except ImportError:
            import importlib.util as _ilu057

            _fp = os.path.join(_AGENT_DIR_057, "feat_059.py")
            _sp = _ilu057.spec_from_file_location("feat_059", _fp)
        ...
        try:
            ...
            _mp = os.path.join(_dir057, "model_059.txt")
            ...
            _z = _np057.load(os.path.join(_dir057, "model_059.npz"))
...
# ---- EXP-059: DISCARD二値分類器（捨てるスコア）。model_discard_059.* があれば有効 ----
_MD059 = None
if os.environ.get("F059_DISCARD", "on") != "off" and _M057 is not None:
    try:
        _dp = os.path.join(_AGENT_DIR_057, "model_discard_059.txt")
        try:
            ...
            _z = _npd.load(os.path.join(_AGENT_DIR_057, "model_discard_059.npz"))
