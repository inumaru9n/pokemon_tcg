"""185_alakazam_v8 — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: Yushin Ito + AlphaStarmie / 156952a871 / 07-17〜08-07（170と同一データ）

**EXP-122: 語彙の基準と正規化方針を正した版**（104から board 506→458）:
- 語彙 **73枚**: 枠A 70（① ARCH_DECK 1〜2系統 ∧ ② **正例数 >= k·d_model**、k=17）
  ＋ 枠B 3（①' 3系統以上 ∧ ④効果テキストが我々を対象 ∧ ⑤有界 ∧ ⑥既存特徴と非重複）
  ②は mu でなく**正例数**で測る。1列は隠れ256ユニットへ繋がるので256本の重みを推定し、
  その1本あたりの標本数が判定量になる（mu は1枚積みの札を不当に落とす）
  Boss's Orders は `opp_used_boss` と定義上同一なので⑥で除外、Tool Scrapper は②で落ちる
...
model_185.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[185_alakazam_v8] class_bias.json を読めない: {_e}", file=sys.stderr)
...
if os.environ.get("NN_170", "on") != "off":
    try:
        import feat_185 as _ft
        from nn_185 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_185.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_185.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[185_alakazam_v8] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[185_alakazam_v8] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[185_alakazam_v8] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[185_alakazam_v8] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
