"""099_alakazam_gen — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: Yushin Ito / sig 156952a871 / 07-17〜07-25（3,593試合）。094と同一教師・同一データで、特徴器のみ自動生成版に差し替えた比較用（EXP-096）
...
model_099.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
_NET = None
_FT = None
if os.environ.get("NN_099", "on") != "off":
    try:
        import feat_099 as _ft
        from nn_099 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_099.npz")
        ...
        import traceback
        print(f"[099_alakazam_gen] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[099_alakazam_gen] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[099_alakazam_gen] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
