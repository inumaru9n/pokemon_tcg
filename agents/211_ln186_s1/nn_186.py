...
class LSTMPolicy:
    ...
    def _advance(self, board, logs, prev, ctx: int):
        ...
        # **相手の直前の選択**（EXP-133）。prev が6要素なら後半3を同じ埋め込みで引く。
        # 3要素の旧モデルとも共存できるよう、重みの入力次元で分岐する。
        _use_opp = len(prev) >= 6 and self.w["enc.0.weight"].shape[1] > (
            len(board) + len(logs) + 16 + e_prev.shape[0] + 2)
        e_pop = (self.w["prev_card_emb.weight"][self._cidx(prev[3])]
                 if _use_opp else None)
        ...
        _parts = [bd, np.asarray(logs, np.float32), e_ctx, e_prev,
                  np.asarray(prev[1:3], np.float32)]
        if e_pop is not None:
            _parts += [e_pop, np.asarray(prev[3:6][1:3], np.float32)]
        x = np.concatenate(_parts).astype(np.float32)
        h = self._lstm_step(self._mlp(x, "enc"))
        if "h_norm.weight" in self.w:            # LSTM出力の LayerNorm（EXP-134）
            _g = self.w["h_norm.weight"]; _b = self.w["h_norm.bias"]
            h = (h - h.mean()) / np.sqrt(h.var() + 1e-5) * _g + _b
        return h
    ...
    def _mlp(self, x, prefix: str, n_layers: int = 2):
        """Linear→ReLU（→LayerNorm）を保存順に適用する（Dropoutは推論では恒等）。

        **重みの次元で層種を判定する**（EXP-134）。`_lin_idx` は
        `{prefix}.N.weight` を全部拾うので、LayerNorm を Sequential に入れると
        1次元パラメータが Linear として扱われ、shape不一致で無言フォールバックに落ちる。
        Linear は2次元 / LayerNorm は1次元、という不変量で分岐する。
        学習側の並びは Linear → ReLU → LayerNorm → Dropout なので、
        保存順にこの分岐を当てれば torch と同じ計算になる。
        """
        for i in self._lin_idx(prefix):
            W = self.w[f"{prefix}.{i}.weight"]
            b = self.w[f"{prefix}.{i}.bias"]
            if W.ndim == 1:                      # LayerNorm
                m = x.mean(); v = x.var()
                x = (x - m) / np.sqrt(v + 1e-5) * W + b
            else:                                # Linear → ReLU
                x = x @ W.T + b
                x = np.maximum(x, 0.0)
        return x
    ...
    def scores(self, board, logs, prev, ctx: int, opt_cids, opt_vecs,
        ...
        board: (F,) / logs: (L,) / prev: (3,) または (6,) / opt_vecs: (O, D)
