...
Yushin(81f1)の全MAIN決定（ctx0・min=max=1・選択肢2以上）について
  - feat_062.featurize のゾーン特徴（~120次元）
  - 選択可能な行動クラスのマスク / 本人が選んだクラス
  - 052ベースの予測クラスとoption一致（比較基準）
を記録して pickle 保存する。列挙なし=軽量。

usage:
  uv run python agents/062_majkel_058arch/collect_062.py \
...
AGENT_DIR = os.path.join(REPO_ROOT, "agents/062_majkel_058arch")
...
def _ensure_worker():
    ...
    spec = importlib.util.spec_from_file_location(
        "agent062", os.path.join(AGENT_DIR, "main.py"))
