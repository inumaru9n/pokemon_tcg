...
collect_062.py の pickle から (ゾーン特徴 → 本人の行動クラス) の多クラス分類を
複数モデルで学習・比較する。推論は「選択可能クラスでマスクした argmax」=
実運用と同じ形で評価する。

比較基準（ベース）= 052policyの選択クラス（pb_cls）。
usage:
  uv run python agents/062_majkel_058arch/train_062.py \
      --dev dev.pkl --val val.pkl --models gbt logreg mlp --save-best model_062
"""
