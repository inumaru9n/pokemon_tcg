"""051_alakazam_deck — 045_alakazam_full のデッキ変更スイープ (I-090 / EXP-051)。

main.py は 045 のコピー + 以下の最小差分のみ（方策・探索は不変）:
- agent() 冒頭で _search_time_used をゲーム毎リセット（045の潜在バグ修正、050と同じ）
- 新カード2種の最小プレイ条件（該当カードがデッキに無いバリアントでは不活性）:
  - Tool Scrapper(1137): 相手の場にツールが1枚以上あるときのみプレイ、
    ツール選択(TOOL_CARD)は相手側のみ（038と同実装）
  - Battle Cage(1264): 自分のスタジアムが場に無いときプレイ（b7cef頻度ルール、EXP-047）
deck.csv をバリアント（deck_v1〜v4.csv）で差し替えて評価した。ベースは deck_base.csv。
**採用=V3（deck.csv=deck_v3.csv）: Xerosic 3→4 / Nighttime Mine 3→2**（EXP-051 validated:
vs045直接800戦55.5%[52.0-58.9]、プールv3 54.9%[51.7-58.0]）。
V1 Tool Scrapper / V2 Hammer4 / V4 Battle Cage は rejected（Tool Scrapper/Battle Cageの
プレイ条件コードはV3デッキでは不活性のまま残置）。
