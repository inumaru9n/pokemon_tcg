...
class LSTMPolicy:
    ...
    def __init__(self, path: str):
        ...
        # **LSTMが無いモデル（EXP-140 の L0）にも対応する**。d_model は
        # proj の入力次元から取れる（LSTMの有無に依らず存在する）。
        self.h_dim = (self.w["lstm.weight_hh_l0"].shape[1] if self.n_layers
                      else int(self.w["proj.weight"].shape[1]))
        # 再帰の代わりに MLP を挟む変種（post.N.weight）
        self.has_post = any(k.startswith("post.") for k in self.w)
    ...
    def _advance(self, board, logs, prev, ctx: int):
        ...
        z = self._mlp(x, "enc")
        if self.n_layers:
            return self._lstm_step(z)
        # **再帰なし**: 決定ごとに独立。状態を持たないので reset は無害な空操作
        return self._mlp(z, "post") if self.has_post else z
