...
# ロード順: lightgbm+model_070.txt（ローカル高速）→ model_070.npz（純Python、Kaggle用。
...
if os.environ.get("F057_ML", "on") != "off":
    try:
        ...
        try:
            ...
            _mp = os.path.join(_dir057, "model_070.txt")
            ...
            _z = _np057.load(os.path.join(_dir057, "model_070.npz"))
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # EXP-072: 各レイヤに自分の前提条件を持たせる。turn>=2 は lethal探索の前提
    # （turn 2以前にKOは成立しない）であってMLレイヤの前提ではない。070まではMLレイヤが
    # このガードに相乗りしていたため turn 1 でモデルが呼ばれていなかった（EXP-071で発見）。
