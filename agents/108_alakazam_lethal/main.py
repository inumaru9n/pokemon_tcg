"""108_alakazam_lethal — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。
...
model_104.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
from cg.api import OptionType as _OptionType, to_observation_class  # noqa: E402
...
try:
    ...
    print(f"[108_alakazam_lethal] class_bias.json を読めない: {_e}", file=sys.stderr)

# ---- モデルと特徴のロード（失敗しても落ちない） ----
_NET = None
_FT = None
if os.environ.get("NN_104", "on") != "off":
    try:
        import feat_104 as _ft
        from nn_104 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_104.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_104.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[108_alakazam_lethal] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[108_alakazam_lethal] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
# ---- lethal層: そのターンで勝てる詰みを取り逃さない ----
# 実測（tools/lethal_diag.py）で 104 は詰みを **27.4% 見逃していた**（62回中17回）。
# 打点計算の KO判定は真値と100%一致している（EXP-099）ので、**探索なしで確実に判定できる**。
# search_begin を使わないため、本番環境で探索APIが動くかという懸念とも無関係。
_ATK_IDX = None


def _attack_index_map():
    """attackId -> (種族cardId, ATTACKS_056内のindex)。CLS_ATK の命名から復元する。"""
    global _ATK_IDX
    if _ATK_IDX is None:
        by_name = {v: k for k, v in _FT.SPECIES_NAME.items()}
        m = {}
        for aid, nm in getattr(_FT, "CLS_ATK", {}).items():
            g = re.match(r"^atk_(.+)_(\d+)$", nm)
            if g and g.group(1) in by_name:
                m[aid] = (by_name[g.group(1)], int(g.group(2)))
        _ATK_IDX = m
    return _ATK_IDX


def _lethal_option(obs):
    """そのターンで勝てる攻撃の選択肢indexを返す。無ければ None。

    条件: (1) 相手アクティブをKOできる打点がある (2) そのKOで取るサイド >= 自分の残りサイド
    打点は弱点×2・抵抗−30・効果無効化（Articuno/Mist系）を織り込んだ値を使う。
    """
    try:
        st = obs.current
        sel = obs.select
        if sel is None or int(sel.context) != 0:
            return None
        me = st.players[st.yourIndex]
        op = st.players[1 - st.yourIndex]
        act = me.active[0] if me.active and me.active[0] else None
        oa = op.active[0] if op.active and op.active[0] else None
        if act is None or oa is None or oa.hp <= 0:
            return None
        # 勝ちに必要なサイド枚数を満たすか
        if len(me.prize) > _FT._prz_of_id(oa.id):
            return None
        opf = [oa] + [x for x in op.bench if x is not None]
        myf = [act] + [x for x in me.bench if x is not None]
        ctx = _FT._atk_ctx(me, act)
        et = _FT._etypes(act)
        acd = _FT.card_table.get(act.id)
        wmul, rsub = _FT._wr_mult(getattr(acd, "energyType", None), _FT._cd(oa))
        amap = _attack_index_map()
        atks = _FT.ATTACKS_056.get(act.id, ())
        best = None
        for i, o in enumerate(sel.option):
            if o.type != _OptionType.ATTACK:
                continue
            t = amap.get(o.attackId)
            if t is None or t[0] != act.id or t[1] >= len(atks):
                continue
            cn, dmg, mv = atks[t[1]]
            if not _FT._afford(act.id, mv, cn, len(act.energyCards or []), et):
                continue
            d, ok, is_cnt = _FT._resolve_dmg(act.id, mv, dmg, ctx)
            if not ok or d <= 0:
                continue
            if _FT._protected(oa, False, opf, acd, is_cnt):
                continue
            d = max(0, d * wmul - rsub)
            if d >= oa.hp and (best is None or d > best[1]):
                best = (i, d)
        return None if best is None else best[0]
    except Exception:  # noqa: BLE001
        return None      # 判定に失敗したら黙って通常の方策へ
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # **詰みは方策に任せない**。勝てる盤面で勝ちを取り逃す率が実測27.4%あった。
    _li = _lethal_option(obs)
    if _li is not None:
        _prev = [float(_FT.resolve_option(obs, obs.select.option[_li],
                                          obs.current.yourIndex)[0]),
                 float(obs.select.option[_li].type or -1),
                 float(obs.select.option[_li].area or -1)]
        globals()["_prev"] = _prev
        return [_li]
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[108_alakazam_lethal] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[108_alakazam_lethal] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
