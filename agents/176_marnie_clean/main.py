"""176_marnie_clean — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: **Luca / 1ec0f47981 / 2026-07-16〜08-06 / 全5,458試合**（勝率58.3%・483,798決定）

**プールのMarnie枠（161）を174と同じ設計で作り直したもの**。ExItの相手役に使う。
...
model_176.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[176_marnie_clean] class_bias.json を読めない: {_e}", file=sys.stderr)
...
if os.environ.get("NN_170", "on") != "off":
    try:
        import feat_176 as _ft
        from nn_176 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_176.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_176.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[176_marnie_clean] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[176_marnie_clean] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[176_marnie_clean] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[176_marnie_clean] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
