...
デッキ: agents/176_marnie_clean/deck.csv
...
# ==== 汎用札の脅威追跡（枠B。EXP-122）====
# **META_CARDS とは目的が違う**。①（識別）の補集合＝3系統以上に入る汎用札のうち、
# **効果テキストが我々の盤面・手札・どうぐを対象に取るもの**だけを追跡する。
#   ①' 汎用性    ARCH_DECK 9系統のうち **3系統以上**              → 候補17枚
#   ②  学習可能性 教師データでの board_mu >= 0.02（生成後に検証し、割れたら外す）
#   ④  干渉性    skills[].text が `opponent`/`each player` を含む → **5枚**
#   ⑤  有界性    同名4枚上限（Basic {G} はここで落ちる）
# 表現は opp_seen_{cid}（観測枚数）のみ。**残枚数推定・超幾何確率は採らない**
# （EXP-113 軸A。プール -0.12/-0.23pt・対Lopunny -3.2pt で棄却済み）。
# ②の実測（教師データ 前期07-17〜19 / 後期08-02〜07 の両方で確認）:
#   **1182 Boss's Orders は除外**: 既存の `opp_used_boss`（相手トラッシュのBoss枚数）と
#   **定義上同一**だった（トレーナーはトラッシュにしか現れないため）。検証で発見。
#   ＝ 汎用札を足すときは「既に別名で追跡されていないか」を必ず確認すること。
#   1197 Xerosic's Machinations mu 0.627 / 0.352   合格
#   140  Fezandipiti ex         mu 0.253 / 0.143   合格
#   1080 Unfair Stamp (ACE)     mu 0.170 / 0.111   合格
#   1137 Tool Scrapper          mu 0.006 / 0.011   Yushinでは②不合格だが**リストには残す**
#        （②はデータごとに calc_drop_cols が判定する。教師が変われば通りうる）
THREAT_CARDS = [140, 1080, 1137, 1197]

# 有効な特徴グループ（None なら全部。tools/featgen_core.py の _GRULES 参照）
FEAT_GROUPS = {'oppseen', 'misc', 'megaprize', 'selctx', 'resource', 'slot_attr', 'base', 'threat2'}

# **このデッキの教師データで算出した削除列**（完全重複の余剰 + 定数列）。
# tools/calc_drop_cols.py が出力する JSON を --drop で渡して焼き込む。
DROP_COLS = ['act_cant', 'act_dmg', 'act_e', 'act_e_basic_d_en', 'act_fresh', 'act_froslass', 'act_grimmsnarl', 'act_hp_ratio', 'act_impidimp', 'act_morgrem', 'act_munkidori', 'act_n_tool', 'act_snorunt', 'dis_1080', 'hand_1080', 'hand_1259', 'hand_7', 'hand_n_tool', 'left_104', 'left_1079', 'left_1080', 'left_1086', 'left_1097', 'left_112', 'left_1122', 'left_1137', 'left_1152', 'left_1182', 'left_1219', 'left_1227', 'left_1231', 'left_1259', 'left_646', 'left_647', 'left_648', 'left_7', 'left_860', 'me_asleep', 'me_burned', 'me_dot_ability', 'me_paralyzed', 'me_poisoned', 'me_retreat_cost', 'ms0_tool', 'ms1_tool', 'ms2_tool', 'ms3_tool', 'ms4_tool', 'ms5_tool', 'my_act_prize', 'my_has_protect', 'n_deck_me', 'n_deck_op', 'n_munkidori', 'op_asleep', 'op_burned', 'op_confused', 'op_paralyzed', 'op_retreat_cost', 'opp_act_dmg', 'opp_act_energy', 'opp_act_maxdmg', 'opp_act_prize', 'opp_best_dmg_next', 'opp_cant', 'opp_seen_1071', 'opp_seen_1081', 'opp_seen_1118', 'opp_seen_1127', 'opp_seen_1137', 'opp_seen_117', 'opp_seen_1194', 'opp_seen_1201', 'opp_seen_1203', 'opp_seen_1213', 'opp_seen_1218', 'opp_seen_1221', 'opp_seen_1223', 'opp_seen_1245', 'opp_seen_1246', 'opp_seen_1251', 'opp_seen_174', 'opp_seen_849', 'opp_seen_90', 'opp_seen_93', 'os0_i_am_weak_to_it', 'os0_resist_vs_me', 'os0_weak_vs_me', 'os1_resist_vs_me', 'os2_resist_vs_me', 'os3_resist_vs_me', 'os4_resist_vs_me', 'os5_resist_vs_me', 'prz_if_my_act_ko', 'prz_if_opp_act_ko']
...
SPECIES_NAME = {104: 'froslass', 112: 'munkidori', 646: 'impidimp', 647: 'morgrem', 648: 'grimmsnarl', 860: 'snorunt'}
LINE_OF = {104: 'snorunt', 112: 'munkidori', 646: 'impidimp', 647: 'impidimp', 648: 'impidimp', 860: 'snorunt'}        # cardId -> 進化ライン名（最下段の種の名前）
LINE_NAMES = ('impidimp', 'munkidori', 'snorunt')
...
ATTACH_BUCKET = {104: 'snorunt_f', 112: 'munkidori_f', 646: 'impidimp_p', 647: 'impidimp_p', 648: 'impidimp_f', 860: 'snorunt_p'}   # エネ貼り先の種 -> クラス分類のバケット
CLS_PLAY = {104: 'play_froslass', 112: 'play_munkidori', 646: 'play_impidimp', 647: 'play_morgrem', 648: 'play_grimmsnarl', 860: 'play_snorunt', 1079: 'use_rare_candy', 1080: 'use_unfair_sta', 1086: 'use_buddy_budd', 1097: 'use_night_stre', 1122: 'use_pokégear_3', 1137: 'use_tool_scrap', 1152: 'use_poké_pad', 1182: 'use_orders', 1219: 'use_petrel', 1227: 'use_determinat', 1231: 'use_dawn', 1259: 'use_spikemuth_'}
CLS_EVO = {104: 'evo_froslass', 647: 'evo_morgrem', 648: 'evo_grimmsnarl'}
CLS_AB = {104: 'ab_froslass', 112: 'ab_munkidori', 646: 'ab_impidimp', 647: 'ab_morgrem', 648: 'ab_grimmsnarl', 860: 'ab_snorunt', 1259: 'ab_spikemuth_'}        # ポケモンの特性 + 効果を持つスタジアム
CLS_ATK = {131: 'atk_froslass_0', 141: 'atk_munkidori_0', 934: 'atk_impidimp_0', 935: 'atk_impidimp_1', 936: 'atk_morgrem_0', 937: 'atk_grimmsnarl_0', 1239: 'atk_snorunt_0'}      # attackId -> クラス名（技の打ち分け）
CLASSES = ('ab_froslass', 'ab_grimmsnarl', 'ab_impidimp', 'ab_morgrem', 'ab_munkidori', 'ab_other', 'ab_snorunt', 'ab_spikemuth_', 'ab_stadium', 'atk_froslass_0', 'atk_grimmsnarl_0', 'atk_impidimp_0', 'atk_impidimp_1', 'atk_morgrem_0', 'atk_munkidori_0', 'atk_other', 'atk_snorunt_0', 'attach_basic_d_en_impidimp_f', 'attach_basic_d_en_impidimp_p', 'attach_basic_d_en_munkidori_f', 'attach_basic_d_en_oth', 'attach_basic_d_en_snorunt_f', 'attach_basic_d_en_snorunt_p', 'end', 'evo_froslass', 'evo_grimmsnarl', 'evo_morgrem', 'other', 'play_froslass', 'play_grimmsnarl', 'play_impidimp', 'play_morgrem', 'play_munkidori', 'play_snorunt', 'retreat', 'use_buddy_budd', 'use_dawn', 'use_determinat', 'use_night_stre', 'use_orders', 'use_petrel', 'use_poké_pad', 'use_pokégear_3', 'use_rare_candy', 'use_spikemuth_', 'use_tool_scrap', 'use_unfair_sta')
