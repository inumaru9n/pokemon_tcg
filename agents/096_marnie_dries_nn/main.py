"""096_marnie_dries_nn — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: Dries @ Tufa Labs / sig 1ec0f47981 / 07-26〜07-28。特徴器は手作り feat_096.py（092の盤面 + 094のオプション側）
...
model_096.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
_NET = None
_FT = None
if os.environ.get("NN_096", "on") != "off":
    try:
        import feat_096 as _ft
        from nn_096 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_096.npz")
        ...
        import traceback
        print(f"[096_marnie_dries_nn] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[096_marnie_dries_nn] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[096_marnie_dries_nn] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
