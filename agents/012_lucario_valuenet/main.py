...
# ---------------------------------------------------------------------------
# Learned value function (EXP-012).
# Features are extracted at END-OF-TURN states (the same distribution the
# search evaluates). The win-probability model is a logistic regression
# trained on games of this policy vs the pool; weights are baked in below.

VALUE_WEIGHTS: list[float] = [-0.754942, 0.378024, 1.047594, 0.129909, 0.564635, 0.085050, -0.070614, 0.000000, -0.559648, -0.028134, -0.240451, -0.020782, -0.280980, 0.482505, 0.323691, -0.564322, -0.200865, 0.001266, -1.000222, -0.042138, -0.261002, 0.132544, 0.000000, 0.000000, 0.000000, 0.000000]  # empty = value search disabled
VALUE_MEANS: list[float] = [4.078350, 4.761491, 0.683141, 4.991229, 7.183158, 4.391330, 1.384499, 0.000000, 4.327823, 4.871764, 2.624357, 0.745720, 0.268449, 2.077979, 1.471367, 1.337438, 1.075145, 5.955638, 7.678080, 2.272000, 2.549734, 0.765624, 0.000000, 0.000000, 0.000000, 0.000000]
VALUE_STDS: list[float] = [1.649221, 1.543376, 1.745428, 1.172162, 2.601051, 2.921483, 1.160774, 1.000000, 1.329648, 2.454428, 2.618611, 1.011211, 0.626677, 1.346245, 0.822356, 1.073489, 1.061333, 2.115312, 4.240814, 1.134296, 1.179910, 0.335608, 1.000000, 1.000000, 1.000000, 1.000000]
VALUE_BIAS: float = -0.434645
VALUE_MARGIN = 0.05  # override the heuristic only for a clear win-prob gain


def value_features(obs: Observation, my_index: int) -> list[float]:
    """Side-symmetric board features from `my_index`'s perspective."""
    state = obs.current
    me = state.players[my_index]
    op = state.players[1 - my_index]

    def board_stats(ps):
        count = hp = en = ready = stage2 = 0.0
        for p in ps.active + ps.bench:
            ...
            if len(p.energies) >= 2:
                ready += 1
            if card_table[p.id].stage2:
                stage2 += 1
        return count, hp, en, ready, stage2

    m_cnt, m_hp, m_en, m_rdy, m_s2 = board_stats(me)
    o_cnt, o_hp, o_en, o_rdy, o_s2 = board_stats(op)
    ...
    return [
        float(len(me.prize)),
        float(len(op.prize)),
        float(len(op.prize) - len(me.prize)),
        m_cnt, m_hp / 100.0, m_en, m_rdy, m_s2,
        o_cnt, o_hp / 100.0, o_en, o_rdy, o_s2,
        (m_active.hp / 100.0) if m_active is not None else 0.0,
        float(len(m_active.energies)) if m_active is not None else 0.0,
        (o_active.hp / 100.0) if o_active is not None else 0.0,
        float(len(o_active.energies)) if o_active is not None else 0.0,
        float(me.handCount), float(op.handCount),
        me.deckCount / 10.0, op.deckCount / 10.0,
        min(state.turn, 30) / 10.0,
        1.0 if (me.asleep or me.paralyzed) else 0.0,
        1.0 if (op.asleep or op.paralyzed) else 0.0,
        1.0 if me.confused else 0.0,
        1.0 if op.confused else 0.0,
    ]


def value_predict(obs: Observation, my_index: int) -> float:
    """Win probability for my_index (requires baked weights)."""
    import math as _math
    x = value_features(obs, my_index)
    z = VALUE_BIAS
    for i, w in enumerate(VALUE_WEIGHTS):
        s = VALUE_STDS[i] if VALUE_STDS[i] > 1e-9 else 1.0
        z += w * ((x[i] - VALUE_MEANS[i]) / s)
    return 1.0 / (1.0 + _math.exp(-max(-30.0, min(30.0, z))))
...
def _value_search(obs: Observation, policy: LucarioPolicy) -> list[int] | None:
    """Re-rank top MAIN candidates by learned end-of-turn win probability.

    Overrides the heuristic only when a candidate beats the heuristic's own
    choice by VALUE_MARGIN in average win probability.
    """
    global _search_time_used
        ...
        for _ in range(EVAL_SAMPLES):
            ...
            for idx in candidates:
                ...
                try:
                    child = search_step(root.searchId, [idx])
                    final_obs = _rollout(child, root_turn, deadline)
                    if final_obs.current.result != -1:
                        v = 1.0 if final_obs.current.result == my_index else 0.0
                    else:
                        v = value_predict(final_obs, my_index)
                    totals[idx] += v
                    counts[idx] += 1
                except Exception:
                    counts[idx] += 1  # totals += 0.0: crashing line = losing line
    finally:
        try:
            search_end()
        except Exception:
    ...
    if best == top1:
        return None
    if totals[best] / counts[best] > totals[top1] / counts[top1] + VALUE_MARGIN:
        return [best]
    return None
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if obs.select.context == SelectContext.MAIN and obs.current.turn >= 2:
        ...
        try:
            ...
            if choice is None:
                choice = _value_search(obs, policy)
        except Exception:
            choice = None
