"""model_091.txt (lightgbm) → model_091.npz (純Python/numpy推論用) 変換 (EXP-091)。

Kaggle評価環境にlightgbmが無いため、全ツリーをフラット配列に落とす。
検証: val決定データ上で lightgbm予測との argmax一致率を報告する。

usage:
  uv run python agents/091_marnie_ml/export_model_091.py --check agents/091_marnie_ml/val_091.pkl
"""
...
def export():
    ...
    bst = lgb.Booster(model_file=os.path.join(AGENT_DIR, "model_091.txt"))
    dump = bst.dump_model()
    ...
    out = os.path.join(AGENT_DIR, "model_091.npz")
    np.savez_compressed(
        out,
...
def predict_pure(z, x):
    """純Python推論（main.pyに同じロジック。ここでは検証用）。"""
...
def check(bst, pkl_path, n=3000):
    ...
    z = np.load(os.path.join(AGENT_DIR, "model_091.npz"))
