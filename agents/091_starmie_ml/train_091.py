"""EXP-091 Starmie(491b)選択クラス予測の学習 (I-136)。

collect_091.py の pickle から (ゾーン特徴 → 教師の行動クラス) の多クラス分類を学習。
推論は「選択可能クラスでマスクした argmax」= 実運用と同じ形で評価する。
比較基準（ベース）= 036policyの選択クラス（pb_cls）。
パラメタは train_058 踏襲（LightGBM multiclass）。

usage:
  uv run python agents/091_marnie_ml/train_091.py \
      --dev agents/091_marnie_ml/dev_091.pkl --val agents/091_marnie_ml/val_091.pkl \
      --save-best agents/091_marnie_ml/model_091
"""
...
def report(name, pred, y, pb, mb, turn, classes):
    ...
    for ci in np.argsort(-cnt)[:14]:
        m = y == ci
        if m.sum() < 30:
            continue
        print(f"    {classes[ci]:14s} {m.sum():5d}  {(pred[m]==ci).mean():.3f}  "
              f"{(pb[m]==ci).mean():.3f}")
    return acc
...
def main():
    ...
    ap.add_argument("--models", nargs="+", default=["gbt", "logreg"])
    ...
    print(f"base class-acc : dev={(pbd==yd).mean():.4f} val={(pbv==yv).mean():.4f}")
