"""091_starmie_ml — 036_starmie_lethal + MAIN行動クラスGBT層 (I-136, EXP-091).

EXP-057定式化（行動クラス直接学習）のStarmie水平展開3例目（057=Alakazam、
089=Marnieに続く）。教師=Yushin Ito本人のladderリプレイ（sig 491b8bfb26、
036と同一60枚）。層順: lethal探索（036そのまま）→ MLクラスargmax（MAIN・
turn>=2・min=max=1のみ。クラス内の具体option化は036ルールのスコア）→
036の30ルール。モデル無し / F091_ML=off で036と同一挙動。
"""
...
# ロード順: lightgbm+model_091.txt（ローカル高速）→ model_091.npz（純Python、
# Kaggle用。argmax一致検証済み）→ どちらも無ければ036ルールへ自然フォールバック
try:  # Kaggle評価環境はexecロードのため __file__ が無い
    _AGENT_DIR_091 = os.path.dirname(os.path.abspath(__file__))
except NameError:
    _AGENT_DIR_091 = ("/kaggle_simulations/agent"
                      if os.path.exists("/kaggle_simulations/agent/main.py")
                      else os.getcwd())

_M091 = None
if os.environ.get("F091_ML", "on") != "off":
    try:
        try:
            import feat_091 as _ft091
        except ImportError:
            import importlib.util as _ilu091

            _fp = os.path.join(_AGENT_DIR_091, "feat_091.py")
            _sp = _ilu091.spec_from_file_location("feat_091", _fp)
            _ft091 = _ilu091.module_from_spec(_sp)
            _sp.loader.exec_module(_ft091)
        try:
            import lightgbm as _lgb091

            _mp = os.path.join(_AGENT_DIR_091, "model_091.txt")
            if not os.path.exists(_mp):
                raise FileNotFoundError(_mp)
            _M091 = {"mode": "lgb", "bst": _lgb091.Booster(model_file=_mp),
                     "ft": _ft091}
        except Exception:  # noqa: BLE001  lightgbm無し → npz純Python推論
            import numpy as _np091

            _z = _np091.load(os.path.join(_AGENT_DIR_091, "model_091.npz"))
            _M091 = {"mode": "npz", "ft": _ft091,
...
def _m091_scores(x: list) -> list:
    """行動クラスごとの生スコア（softmax前。argmax用途なので正規化不要）。"""
    if _M091["mode"] == "lgb":
        return list(_M091["bst"].predict([x])[0])
    feat, thr = _M091["feat"], _M091["thr"]
    left, right, val = _M091["left"], _M091["right"], _M091["val"]
    scores = [0.0] * _M091["ncls"]
    for r, c in zip(_M091["roots"], _M091["tcls"]):
        i = int(r)
        while feat[i] >= 0:
