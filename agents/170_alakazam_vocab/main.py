"""170_alakazam_vocab — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: Yushin Ito + AlphaStarmie / 156952a871 / 07-17〜08-07（語彙を教師データ基準で選定・oparch除外）
...
model_170.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[170_alakazam_vocab] class_bias.json を読めない: {_e}", file=sys.stderr)
...
if os.environ.get("NN_170", "on") != "off":
    try:
        import feat_170 as _ft
        from nn_170 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_170.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_170.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[170_alakazam_vocab] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[170_alakazam_vocab] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[170_alakazam_vocab] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[170_alakazam_vocab] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
