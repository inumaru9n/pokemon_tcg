"""175_alakazam_clean_win — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: Yushin Ito + AlphaStarmie / 156952a871 / 07-17〜08-07（**勝ち試合のみ 2,956試合 / 222,994決定 = 全体の51.2%**）

**174と特徴器・語彙・clip・アーキテクチャが1ビットも同一**で、教師データだけが違う。
勝ち選別の効果を単独で測るための対照（過去3例はすべて負け: 161 -2.0pt / 163 -2.4pt / 163直接 -1.2pt）。
...
model_175.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[175_alakazam_clean_win] class_bias.json を読めない: {_e}", file=sys.stderr)
...
if os.environ.get("NN_170", "on") != "off":
    try:
        import feat_175 as _ft
        from nn_175 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_175.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_175.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[175_alakazam_clean_win] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[175_alakazam_clean_win] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[175_alakazam_clean_win] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[175_alakazam_clean_win] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
