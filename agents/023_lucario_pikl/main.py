...
EVAL_CANDIDATES = 4
EVAL_SAMPLES = 2
PIKL_KAPPA = float(os.environ.get("PIKL_KAPPA", "1.0"))  # anchor strength; swept via env in EXP-023
...
def _eval_search_2ply(obs: Observation, policy: LucarioPolicy) -> list[int] | None:
    ...
    if len(evaluated) < 2:
        return None
    # I-051/EXP-023: piKL soft anchor (arXiv 2112.07544). Pick argmax over candidates of
    #   Q_search(a) + kappa * 10000 * h_norm(a)
    # where h_norm in [0,1] is the heuristic score normalized among candidates and kappa is
    # "prizes worth of heuristic preference". kappa large -> follow the heuristic (=007 safe),
    # kappa=0 -> raw 2-ply search (=008, which lost the mirror). Intermediate = piKL: only
    # deviate from the heuristic when the search advantage clearly outweighs the anchor.
    ...
    def blended(i):
        ...
        return totals[i] / counts[i] + PIKL_KAPPA * 10000.0 * h_norm
