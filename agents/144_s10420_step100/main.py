...
model_144.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
if os.environ.get("NN_104", "on") != "off":
    try:
        import feat_144 as _ft
        from nn_144 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_144.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_144.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
