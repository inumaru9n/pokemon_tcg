...
def _rollout_terminal(state, deadline: float, cap: int = 400):
    """Play both sides with the heuristic until the game ends (or cap/deadline).

    Returns the observation of the terminal (or cut-off) state. Unlike _rollout,
    this does NOT stop at our turn end — it plays to a win/loss so the leaf signal
    is the exact game outcome (the ISMCTS insight: terminal signal, not static eval).
    """
    for _ in range(cap):
        obs = state.observation
...
ISMCTS_CANDIDATES = 3
ISMCTS_ROLLOUTS = 4          # determinizations per candidate
ISMCTS_KAPPA = 0.15          # heuristic anchor (win-rate units) — deviate only on clear edge


def _mc_search(obs: Observation, policy: LucarioPolicy) -> list[int] | None:
    """ISMCTS-lite: flat Monte Carlo with TERMINAL rollouts + heuristic anchor.

    For each top heuristic candidate, run K determinized rollouts to a win/loss and
    take the win rate as Q(a). Pick argmax [ Q(a) + kappa * h_norm(a) ] so the search
    only overrides the heuristic on a clear terminal-win edge. Generalizes lethal
    search (all-win) to non-lethal turns using the exact outcome signal.

    Caveat (documented): the opponent deck is a placeholder in the determinization,
    so rollouts are vs a weak opponent and Q is biased toward self-advancing lines.
    """
    ...
    candidates = ranked[:ISMCTS_CANDIDATES]
    top1 = candidates[0]

    wins = {i: 0.0 for i in candidates}
    tried = {i: 0 for i in candidates}
    try:
        for _ in range(ISMCTS_ROLLOUTS):
            if time.perf_counter() > deadline:
                break
            ...
            for idx in candidates:
                ...
                try:
                    ...
                    final_obs = _rollout_terminal(child, deadline)
                    r = final_obs.current.result
                    wins[idx] += 1.0 if r == my_index else 0.5 if r == 2 else 0.0
                    tried[idx] += 1
                except Exception:
                    tried[idx] += 1  # crash = losing line (win credit 0)
    finally:
        try:
            search_end()
        except Exception:
    ...
    evaluated = [i for i in candidates if tried[i] == ISMCTS_ROLLOUTS]
    if top1 not in evaluated or len(evaluated) < 2:
        return None
    hs = [heuristic_scores[i] for i in evaluated]
    hmin, hmax = min(hs), max(hs)
    span = (hmax - hmin) or 1.0

    def blended(i):
        q = wins[i] / tried[i]
        h_norm = (heuristic_scores[i] - hmin) / span
        return q + ISMCTS_KAPPA * h_norm

    best = max(evaluated, key=blended)
    return None if best == top1 else [best]
