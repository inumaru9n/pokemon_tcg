...
# ============================================================================
# EXP-061: universal imitation model（team=Majkel1337 / sig=156952a871 模倣）
#   全選択肢を「選ばれるスコア」で採点しargmax/top-k。052 policyは使わない。
#   層: lethal（厳密勝ち切り）→ universal model → エンジン順フォールバック。
# ============================================================================
_MU061 = None
if os.environ.get("F061_MODEL", "on") != "off":
    try:
        try:
            import feat_061 as _ft061
        except ImportError:
            import importlib.util as _ilu061
            _fp = os.path.join(_AGENT_DIR_057, "feat_061.py")
            _sp = _ilu061.spec_from_file_location("feat_061", _fp)
            _ft061 = _ilu061.module_from_spec(_sp)
            _sp.loader.exec_module(_ft061)
        try:
            import lightgbm as _lgb061
            _mp = os.path.join(_AGENT_DIR_057, "model_universal_061.txt")
            if not os.path.exists(_mp):
                raise FileNotFoundError(_mp)
            _MU061 = {"mode": "lgb", "bst": _lgb061.Booster(model_file=_mp),
                      "ft": _ft061}
        except Exception:  # noqa: BLE001  npz純Python
            import numpy as _np061
            _z = _np061.load(os.path.join(_AGENT_DIR_057, "model_universal_061.npz"))
            _MU061 = {"mode": "npz", "ft": _ft061,
                      "feat": _z["feat"], "thr": _z["thr"], "left": _z["left"],
                      "right": _z["right"], "val": _z["val"], "roots": _z["roots"]}
    except Exception:  # noqa: BLE001  モデル無し → フォールバックのみ
        _MU061 = None


def _mu061_score(x: list) -> float:
    if _MU061["mode"] == "lgb":
        return float(_MU061["bst"].predict([x])[0])
    feat, thr = _MU061["feat"], _MU061["thr"]
    left, right, val = _MU061["left"], _MU061["right"], _MU061["val"]
    tot = 0.0
    for r in _MU061["roots"]:
        i = int(r)
        while feat[i] >= 0:
...
def _universal_layer(obs: Observation) -> list[int] | None:
    """任意コンテキスト: 全選択肢を採点し、上位k枚（k=minCount）を返す。"""
    select = obs.select
    nopt = len(select.option)
    if nopt < 1:
        return None
    k = max(select.minCount, min(1, select.maxCount))
    if k < 1 or k > nopt:
        return None
    state = obs.current
    my = state.yourIndex
    ...
    scored = []
    for i in range(nopt):
        x, _keys = ft.universal_row(state, select, i, my)
        scored.append((_mu061_score(x), i))
...
def agent(obs_dict: dict) -> list[int]:
    ...
    sel = obs.select
    if len(sel.option) < 2 or sel.minCount >= len(sel.option):
        return _fallback_selection(sel)

    # 層1: lethal（厳密勝ち切り検証。模倣でなく正しさの保証）
    if sel.context == SelectContext.MAIN and obs.current.turn >= 2:
        try:
    ...
    # 層2: universal imitation model（全選択肢を採点）
    if _MU061 is not None:
        try:
            choice = _universal_layer(obs)
        except Exception:
            choice = None
        if choice is not None:
            return choice

    # 層3: エンジン順フォールバック（良→悪順。ランダムより安全）
    return _fallback_selection(sel)
