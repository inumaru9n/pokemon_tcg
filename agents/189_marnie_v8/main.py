"""189_marnie_v8 — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: **Luca / 1ec0f47981 / 2026-07-16〜08-06 / 全5,458試合**（勝率58.3%・483,798決定）

**プールのMarnie枠を186と同じ設計で作り直したもの**（176の後継）。ExItの相手役に使う。
176からの変更は2点: ②を mu>=0.02 から **正例数 >= k*d_model（k=10）** に、
有界カウントの標準化をやめて固定尺度に（**最大 1/sd 111.5 → 15.5**）。
語彙は 枠A 73 + 枠B 3 = 76枚。①④⑤⑥はグローバル、②だけを Luca のデータで判定した。
161にはEXP-113で棄却した軸A 47特徴が混入しており（EXP-120で判明した自動混入の範囲）、
clip=10.0 も入っていた。176ではどちらも無い。
...
model_189.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[189_marnie_v8] class_bias.json を読めない: {_e}", file=sys.stderr)
...
if os.environ.get("NN_170", "on") != "off":
    try:
        import feat_189 as _ft
        from nn_189 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_189.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_189.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[189_marnie_v8] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[189_marnie_v8] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[189_marnie_v8] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[189_marnie_v8] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
