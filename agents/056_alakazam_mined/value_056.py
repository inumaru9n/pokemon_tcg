...
eval: calib_056_decisions.py collect の決定pickle上で、V選択の一致率/fire精度を
  052ベースと比較（dtype・ターン帯別）。win終端の厳密優先も運用と同一に適用。

usage:
  uv run python agents/056_alakazam_mined/value_056.py collect \
      --dates 2026-07-08 2026-07-09 2026-07-11 --out /tmp/tv_dev.npz -w 4
  uv run python agents/056_alakazam_mined/value_056.py fit \
      --train /tmp/tv_dev.npz --val /tmp/tv_val.npz \
      --save agents/056_alakazam_mined/value_055.npz
  uv run python agents/056_alakazam_mined/value_056.py eval \
      --pkl /tmp/dec_val.pkl --value agents/056_alakazam_mined/value_055.npz
"""
...
def _ensure_worker():
    ...
    for p in (REPO_ROOT, AGENT_DIR):
        if p not in sys.path:
            sys.path.insert(0, p)
    spec = importlib.util.spec_from_file_location("value056_agent", main_path)
...
def cmd_eval(args):
    """決定pickle（calib_056_decisions.py collect産）上でV選択の一致率/fire精度。"""
