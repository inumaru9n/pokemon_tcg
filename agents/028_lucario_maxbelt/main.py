...
class LucarioPolicy:
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    # I-057/EXP-028: Maximum Belt (Tool, ACE SPEC) adds +50 to attacks against
                    # the opponent's ACTIVE ex/megaEx, applied BEFORE weakness (engine:
                    # SetProperty.h CalcDamage, damageChangeEx). Folded into the KO check ONLY,
                    # not the general scoring, so targeting in non-KO cases is unchanged
                    # (EXP-019 pattern, avoids mirror regression). Bench targets are fine too:
                    # our gust plan brings them to the Active before the attack lands.
                    ko_damage = damage
                    if (op_data.ex or op_data.megaEx) and any(
                        tool.id == C.MAXIMUM_BELT for tool in my_pokemon.tools
                    ):
                        ko_damage += 100 if op_data.weakness == EnergyType.FIGHTING else 50

                    score = target_score(op_pokemon)
                    prize = prize_count(op_pokemon) if op_pokemon.hp <= ko_damage else 0
                    if prize == 0:
                    ...
                    if score > best_score:
                        ...
                        plan = AttackPlan(
                            ...
                            remain_hp=op_pokemon.hp - ko_damage,
    ...
    def _score_attach(self, option) -> float:
        ...
        if card.id == C.MAXIMUM_BELT:
            # Attach to the attack plan's Pokemon, preferring the Mega Lucario ex line
            # (Mega Brave 270 -> 320 vs Active ex: OHKOs Grimmsnarl ex / Dragapult ex HP320).
            # Tools persist through evolution, so Riolu is a fine early carrier.
            score = 7000
            if pokemon.id == C.MEGA_LUCARIO_EX:
                score += 200
