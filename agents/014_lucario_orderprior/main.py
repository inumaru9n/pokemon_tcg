...
MEGA_BRAVE = 983
LOW_DECK_COUNT = 10
# エンジンの選択肢列挙は強い良→悪順（B1がランダムに88-90%勝つ、Discussion 713608）。
# その順序を弱い事前分布としてスコアに混ぜる。λ=0で007と完全同一（同点時は
# 安定ソートが既にエンジン順を保つため、タイブレークとしては元から有効）。
...
class LucarioPolicy:
    ...
    def choose(self) -> list[int]:
        ...
        scores = [self._score_option(option) - i * ORDER_PRIOR_LAMBDA
