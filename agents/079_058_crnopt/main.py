"""079_058_crnopt — 058 + クラスlogitバイアスのオンライン較正 (I-122 / EXP-079)。

ベース: 058_alakazam_policyml2 の完全コピー。変更点は2つだけ:
 (1) _m057_scores: lightgbm経路を raw_score=True に変更（npz純Python推論のlogitスケールと
     統一。softmaxは単調なのでバイアス0なら058とargmax完全一致）
 (2) _ml_layer: クラスlogitに較正バイアス b[class][bucket] を加算してからargmax。
     bucket=自分の山残量 (>=30 / 15-29 / <=14) の3段。
バイアスは bias_079.json（{クラス名: [b_early, b_mid, b_late]}、欠キーは0）から読む。
環境変数 F079_BIAS=path で差し替え可（SPSA最適化ループ用）。ファイル無し=全0=058と同一。
バイアス値はCRN改造エンジン（cg/Export_seeded.cpp）上のプール対戦勝率SPSAで較正する。
...
# ---- EXP-079: クラスlogitバイアス（CRN-SPSA較正） ----
# {クラス名: [b_early, b_mid, b_late]}。bucket=自分の山残量 >=30 / 15-29 / <=14。
# F079_BIAS=path で外部ファイル指定（最適化ループ用）、無指定なら bias_079.json、
# どちらも無ければ全0（=058と完全同一挙動）。
_BIAS079 = None
try:
    import json as _json079

    _bp079 = os.environ.get("F079_BIAS") or os.path.join(
        _AGENT_DIR_057, "bias_079.json")
    if os.path.exists(_bp079):
        with open(_bp079) as _f079:
            _BIAS079 = {str(k): [float(v[0]), float(v[1]), float(v[2])]
                        for k, v in _json079.load(_f079).items()}
        if not any(any(v) for v in _BIAS079.values()):
            _BIAS079 = None  # 全0はバイアス無しと同義
except Exception:  # noqa: BLE001
    _BIAS079 = None


def _bias_bucket_079(state, my: int) -> int:
    d = state.players[my].deckCount
    return 0 if d >= 30 else (2 if d <= 14 else 1)


def _m057_scores(x: list) -> list:
    """行動クラスごとの生スコア（logitスケール。argmax用途なので正規化不要）。

    EXP-079: lightgbm経路も raw_score=True でnpz推論（葉値の総和=生logit）と同一
    スケールに統一。バイアス加算をスケール非依存にするため。
    """
    if _M057["mode"] == "lgb":
        return list(_M057["bst"].predict([x], raw_score=True)[0])
...
def _ml_layer(obs: Observation) -> list[int] | None:
    ...
    if _BIAS079 is not None:  # EXP-079: クラスlogitバイアス加算
        _bk = _bias_bucket_079(state, my)
        for _cn, _bv in _BIAS079.items():
            _ci = ft.CLASS_ID.get(_cn)
            if _ci is not None and _ci < len(proba):
                proba[_ci] += _bv[_bk]
