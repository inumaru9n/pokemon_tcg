...
MEGA_BRAVE = 983
LOW_DECK_COUNT = 10
ANTIKAZAM_BONUS = 500  # anti-Alakazam targeting bonus (swept in EXP-011)
...
class LucarioPolicy:
    ...
    def _opponent_is_alakazam_deck(self) -> bool:
        # Abra / Kadabra / Alakazam / Dunsparce / Dudunsparce
        return self._opponent_has({741, 742, 743, 305, 66})

    def _matchup_target_bonus(self, pokemon: Pokemon) -> int:
        """Anti-Alakazam gated tech (coefficient tuned by sweep, see EXP-011).

        Their engine is pre-evolutions (Kadabra/Abra) and draw support
        (Dunsparce line, fighting-weak = easy KOs for us). Powerful Hand
        places counters (no weakness), so the race is about starving their
        board, not surviving one big hit.
        """
        if ANTIKAZAM_BONUS <= 0 or not self._opponent_is_alakazam_deck():
            return 0
        if pokemon.id == 742:  # Kadabra
            return ANTIKAZAM_BONUS
        if pokemon.id == 741:  # Abra
            return ANTIKAZAM_BONUS * 7 // 10
        if pokemon.id == 66:   # Dudunsparce (fighting-weak draw engine)
            return ANTIKAZAM_BONUS * 7 // 10
        if pokemon.id == 305:  # Dunsparce
            return ANTIKAZAM_BONUS * 4 // 10
        return 0
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    score = target_score(op_pokemon) + self._matchup_target_bonus(op_pokemon)
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id in {C.MAKUHITA, C.HARIYAMA}:
            ...
            # vs Alakazam they KO an attacker every turn: keep a charged backup
            if self.has_ready_lucario_line and not self._opponent_is_alakazam_deck():
                score -= 50
        return score
    ...
    def _score_active_choice(self, option, card: Pokemon | Card) -> float:
        ...
        if option.playerIndex != self.my_index:
            base = 100 if option.index == plan.target - 1 else 0
            return base + self._matchup_target_bonus(card) / 10
    ...
    def _score_attach(self, option) -> float:
        ...
        if card.id == C.HERO_CAPE:
            score = 7000
            if self._opponent_is_water_deck() or self._opponent_is_alakazam_deck():
                # vs Alakazam: +100 HP = +5 hand cards needed for Powerful Hand,
                # and Riolu survives Fezandipiti's Cruel Arrow (100 snipe)
                if pokemon.id == C.RIOLU:
                    return 12200
                if pokemon.id == C.MEGA_LUCARIO_EX:
                    return 12800
