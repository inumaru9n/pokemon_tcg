"""104_alakazam_feat4 — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。
...
model_155_s{1,2,3}.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[104_alakazam_feat4] class_bias.json を読めない: {_e}", file=sys.stderr)
...
if os.environ.get("NN_104", "on") != "off":
    try:
        import feat_155 as _ft
        from nn_155 import EnsemblePolicy

        # **学習シードだけが違う3本の確率平均**（EXP-117）。データ・特徴器・HPは同一で、
        # 変わるのは重み初期化とバッチ順のみ。104 = s2（3本のval最良）。
        # 1本でも欠けたら黙って弱くならないよう、3本揃っていることを要求する。
        _MODELS = [os.path.join(_AGENT_DIR, f"model_155_{s}.npz")
                   for s in ("s1", "s2", "s3")]
        _missing = [p for p in _MODELS if not os.path.exists(p)]
        if _missing:
            raise RuntimeError(f"アンサンブルのメンバーが欠けている: {_missing}")
        _NET = EnsemblePolicy(_MODELS)
        if _NET is not None:
            _FT = _ft
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_155.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[104_alakazam_feat4] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[104_alakazam_feat4] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[104_alakazam_feat4] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[104_alakazam_feat4] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
