...
# ==== EXP-060: アクティブ選出ランカーの特徴（候補ポケモン×盤面 → 選ぶ） ====
# SWITCH(ctx3)/TO_ACTIVE(ctx4)共通。自ベンチの1体をアクティブに出す選択。

ACTIVE_FEAT_KEYS = None


def _pokemon_feats(state, my_index: int, area, index: int) -> dict[str, float]:
    """候補ポケモン1体の属性（盤面から算出できるもの）。area: 4=active,5=bench。"""
    from cg.api import AreaType
    me = state.players[my_index]
    op = state.players[1 - my_index]
    try:
        p = (me.active[index] if area == AreaType.ACTIVE else me.bench[index])
    ...
    # 種別 one-hot
    for cid in SPECIES:
        d[f"p_is_{SPECIES_NAME[cid]}"] = 1.0 if p.id == cid else 0.0
    psy = sum(1 for e in p.energyCards
              if e.id in (Basic_Psychic_Energy, Telepath_Psychic_Energy))
    d["p_hp"] = p.hp
    d["p_hp_frac"] = p.hp / p.maxHp if p.maxHp else 0.0
    d["p_energy"] = len(p.energyCards)
    d["p_psy"] = psy
    d["p_fresh"] = 1.0 if p.appearThisTurn else 0.0
    # 攻撃準備度（この個体が今攻撃できるか、攻撃準備ランク）
    ready = 0.0
    if p.id == Alakazam and psy >= 1:
        ...
        ready = 3.0
    d["p_ready"] = ready
    # このターン攻撃で撃てる最大打点（自分の攻撃テーブル、PH=手札依存）
    mx, _v = _maxdmg_next(p, len(me.hand or []) - 1)  # 現エネで（+1しない）
    e_now = len(p.energyCards)
    mx0 = 0
    for (cn, dmg, mv) in ATTACKS_056.get(p.id, ()):
        if cn > e_now:
            continue
        if dmg is None:
            if mv == "Powerful Hand":
                mx0 = max(mx0, 20 * len(me.hand or []))
        elif dmg > mx0:
            mx0 = dmg
    d["p_atk_now"] = min(mx0, 990) // 10
    # 逃げエネ（アクティブに出した後の機動力の代理）
    cd = card_table.get(p.id)
    d["p_retreat"] = cd.retreatCost if cd is not None else 0
    d["p_prize"] = _prz_of_id(p.id)  # 出すと取られるプライズ価値
    # 相手アクティブの弱点を突けるか（自分がアクティブになったとき）
    op_act = op.active[0] if op.active else None
    d["p_hits_weak"] = 1.0 if (op_act is not None and cd is not None
                               and card_table.get(op_act.id) is not None
                               and card_table[op_act.id].weakness == cd.energyType) else 0.0
    return d


def active_feat_vector(state, my_index: int, area, index: int, is_switch: int):
    """アクティブ選出ランカー用の (盤面 + 候補ポケモン + 文脈) ベクトル。"""
    global ACTIVE_FEAT_KEYS
    board = featurize(state, my_index)
    pk = _pokemon_feats(state, my_index, area, index)
    merged = {**{f"b_{k}": v for k, v in board.items()}, **pk,
              "ctx_switch": float(is_switch)}
    if ACTIVE_FEAT_KEYS is None:
        ACTIVE_FEAT_KEYS = sorted(merged)
    return [merged.get(k, 0.0) for k in ACTIVE_FEAT_KEYS], ACTIVE_FEAT_KEYS
