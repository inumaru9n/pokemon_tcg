...
# ロード順: lightgbm+model_060.txt（ローカル高速）→ model_060.npz（純Python、Kaggle用。
# argmax一致100%検証済み）→ どちらも無ければ052フォールバック
_M057 = None
if os.environ.get("F057_ML", "on") != "off":
    try:
        try:
            import feat_060 as _ft057
        except ImportError:
            import importlib.util as _ilu057

            _fp = os.path.join(_AGENT_DIR_057, "feat_060.py")
            _sp = _ilu057.spec_from_file_location("feat_060", _fp)
        ...
        try:
            ...
            _mp = os.path.join(_dir057, "model_060.txt")
            ...
            _z = _np057.load(os.path.join(_dir057, "model_060.npz"))
...
# ---- EXP-059: DISCARD二値分類器（捨てるスコア）。model_discard_060.* があれば有効 ----
_MD059 = None
if os.environ.get("F059_DISCARD", "on") != "off" and _M057 is not None:
    try:
        _dp = os.path.join(_AGENT_DIR_057, "model_discard_060.txt")
        try:
            import lightgbm as _lgbd

            if not os.path.exists(_dp):
                raise FileNotFoundError(_dp)
            _MD059 = {"mode": "lgb", "bst": _lgbd.Booster(model_file=_dp),
                      "ft": _M057["ft"]}
        except Exception:  # noqa: BLE001  npz純Python
            import numpy as _npd

            _z = _npd.load(os.path.join(_AGENT_DIR_057, "model_discard_060.npz"))
            _MD059 = {"mode": "npz", "ft": _M057["ft"],
                      "feat": _z["feat"], "thr": _z["thr"], "left": _z["left"],
                      "right": _z["right"], "val": _z["val"], "roots": _z["roots"]}
    except Exception:  # noqa: BLE001  モデル無し → RULE_Dフォールバック
        _MD059 = None


def _md059_score(x: list) -> float:
    """1候補カードの捨てスコア（二値。値の大小のみ使うのでsigmoid不要）。"""
    if _MD059["mode"] == "lgb":
        return float(_MD059["bst"].predict([x])[0])
    feat, thr = _MD059["feat"], _MD059["thr"]
    left, right, val = _MD059["left"], _MD059["right"], _MD059["val"]
    tot = 0.0
    for r in _MD059["roots"]:
        i = int(r)
        while feat[i] >= 0:
            i = int(left[i]) if x[feat[i]] <= thr[i] else int(right[i])
        tot += float(val[i])
...
def _discard_layer(obs: Observation) -> list[int] | None:
    """DISCARD決定: 各候補の捨てスコアを学習器で出し、上位k枚を捨てる。"""
    select = obs.select
    if select.context != SelectContext.DISCARD:
        return None
    k = select.minCount
    if k != select.maxCount or k < 1 or k >= len(select.option):
        return None
    state = obs.current
    my = state.yourIndex
    me = state.players[my]
    ...
    for i, o in enumerate(select.option):
        ...
        scored.append((_md059_score(x), i))
    scored.sort(reverse=True)
    return sorted(idx for _s, idx in scored[:k])


# ---- EXP-060: アクティブ選出ランカー（SWITCH/TO_ACTIVE）。model_active_060.* があれば有効 ----
_MA060 = None
if os.environ.get("F060_ACTIVE", "on") != "off" and _M057 is not None:
    try:
        _ap = os.path.join(_AGENT_DIR_057, "model_active_060.txt")
        try:
            import lightgbm as _lgba

            if not os.path.exists(_ap):
                raise FileNotFoundError(_ap)
            _MA060 = {"mode": "lgb", "bst": _lgba.Booster(model_file=_ap),
                      "ft": _M057["ft"]}
        except Exception:  # noqa: BLE001  npz純Python
            import numpy as _npa

            _z = _npa.load(os.path.join(_AGENT_DIR_057, "model_active_060.npz"))
            _MA060 = {"mode": "npz", "ft": _M057["ft"],
                      "feat": _z["feat"], "thr": _z["thr"], "left": _z["left"],
                      "right": _z["right"], "val": _z["val"], "roots": _z["roots"]}
    except Exception:  # noqa: BLE001  モデル無し → 052フォールバック
        _MA060 = None


def _ma060_score(x: list) -> float:
    if _MA060["mode"] == "lgb":
        return float(_MA060["bst"].predict([x])[0])
    feat, thr = _MA060["feat"], _MA060["thr"]
    left, right, val = _MA060["left"], _MA060["right"], _MA060["val"]
    tot = 0.0
    for r in _MA060["roots"]:
        i = int(r)
        while feat[i] >= 0:
            i = int(left[i]) if x[feat[i]] <= thr[i] else int(right[i])
        tot += float(val[i])
...
def _active_layer(obs: Observation) -> list[int] | None:
    """SWITCH/TO_ACTIVE決定: 各候補ポケモンを選ぶスコアでランクしargmax。"""
    select = obs.select
    ...
    my = state.yourIndex
    ft = _MA060["ft"]
    is_sw = 1 if select.context == SelectContext.SWITCH else 0
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # EXP-059: DISCARD決定は学習器で（RULE_Dの文脈non-modelを置換）
    if _MD059 is not None and obs.select.context == SelectContext.DISCARD:
        try:
            choice = _discard_layer(obs)
        except Exception:
            choice = None
        if choice is not None:
            return choice

    # EXP-060: SWITCH/TO_ACTIVE（アクティブ選出）は学習器で
    if _MA060 is not None and obs.select.context in (
            SelectContext.SWITCH, SelectContext.TO_ACTIVE):
        try:
            choice = _active_layer(obs)
        except Exception:
            choice = None
        if choice is not None:
            return choice
