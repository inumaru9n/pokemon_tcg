"""080_search_ranker — 075 + TO_HAND（サーチ先）二値ランカー (I-118 / EXP-080)。

ベース: 075_058_turn1 の完全コピー。変更点:
 (1) 特徴モジュールを feat_080（=feat_058 + カード属性特徴。MAIN特徴順は完全同一）に変更
 (2) TO_HAND(ctx7)決定に学習層 _tohand_layer を追加: 各候補カード（山/トラッシュ）の
     「取るスコア」を二値GBTで出し上位k枚を選ぶ（059のDISCARD管轄化と同形式）。
     model_tohand_080.{txt,npz} が無い/ F080_TOHAND=off なら従来のRULE_Cスコアに
     フォールバック（=075と完全同一挙動）
 (3) 管轄はk固定（minCount==maxCount）の選択のみ。可変枚数サーチはルール続投
...
if os.environ.get("F057_ML", "on") != "off":
    try:
        try:
            import feat_080 as _ft057
        except ImportError:
            import importlib.util as _ilu057

            _fp = os.path.join(_AGENT_DIR_057, "feat_080.py")
            _sp = _ilu057.spec_from_file_location("feat_080", _fp)
...
# ---- EXP-080: TO_HAND（サーチ先）二値ランカー ----
# ロード順: lightgbm+model_tohand_080.txt → model_tohand_080.npz（純Python）→ 無ければ
# RULE_Cフォールバック（=075と同一挙動）。F080_TOHAND=off で無効化。
_MT080 = None
if os.environ.get("F080_TOHAND", "on") != "off" and _M057 is not None:
    try:
        _tp080 = os.path.join(_AGENT_DIR_057, "model_tohand_080.txt")
        try:
            import lightgbm as _lgbt080

            if not os.path.exists(_tp080):
                raise FileNotFoundError(_tp080)
            _MT080 = {"mode": "lgb", "bst": _lgbt080.Booster(model_file=_tp080),
                      "ft": _M057["ft"]}
        except Exception:  # noqa: BLE001  npz純Python
            import numpy as _npt080

            _z = _npt080.load(os.path.join(_AGENT_DIR_057, "model_tohand_080.npz"))
            _MT080 = {"mode": "npz", "ft": _M057["ft"],
...
def _mt080_score(x: list) -> float:
    """1候補カードの「取る」スコア（二値。大小のみ使うのでsigmoid不要）。"""
    if _MT080["mode"] == "lgb":
        return float(_MT080["bst"].predict([x])[0])
    feat, thr = _MT080["feat"], _MT080["thr"]
    left, right, val = _MT080["left"], _MT080["right"], _MT080["val"]
    tot = 0.0
    for r in _MT080["roots"]:
        i = int(r)
        while feat[i] >= 0:
            i = int(left[i]) if x[feat[i]] <= thr[i] else int(right[i])
        tot += float(val[i])
...
def _tohand_card_id(obs: Observation, opt) -> tuple[int, float] | None:
    """TO_HAND候補option → (カードID, from_discard)。解決不能ならNone。"""
    my = obs.current.yourIndex
    me = obs.current.players[my]
    try:
        if opt.area == AreaType.DECK:
            return obs.select.deck[opt.index].id, 0.0
        if opt.area == AreaType.DISCARD:
            return me.discard[opt.index].id, 1.0
        if opt.area == AreaType.LOOKING:
            return obs.current.looking[opt.index].id, 0.0
    except (IndexError, TypeError, AttributeError):
        return None
    return None


def _tohand_k(select) -> int | None:
    """管轄内なら選ぶ枚数kを返す。k固定(min==max>=1) or 「1枚まで」型(0/1、常に1枚取る
    =教師の実挙動228/228)。それ以外（可変min1/max3等）はNone=ルール続投。"""
...
def _tohand_layer(obs: Observation) -> list[int] | None:
    """TO_HAND決定: 各候補の「取る」スコアを学習器で出し、上位k枚を選ぶ。

    候補が1枚でも解決不能（PRIZE=裏向き等）ならルール続投。"""
    select = obs.select
    if select.context != SelectContext.TO_HAND:
        return None
    k = _tohand_k(select)
    if k is None or k >= len(select.option):
        return None
    state = obs.current
    my = state.yourIndex
    ft = _MT080["ft"]
    ...
    for i, o in enumerate(select.option):
        r = _tohand_card_id(obs, o)
        if r is None:
            return None
        cid, from_disc = r
        x, _keys = ft.to_hand_feat_vector(state, my, cid, from_disc)
        scored.append((_mt080_score(x), i))
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # EXP-075: 058に対するturn1拡張（EXP-072と同一の変更）。turn>=2 は lethal探索の前提で
    # あってMLレイヤの前提ではない。相乗りしていたため turn 1 でモデルが呼ばれていなかった。
    ...
    # EXP-080: TO_HAND（サーチ先）の学習層。失敗時はRULE_Cへ自然フォールバック
    if _MT080 is not None and obs.select.context == SelectContext.TO_HAND:
        try:
            choice = _tohand_layer(obs)
        except Exception:
            choice = None
        if choice is not None:
            return choice
