...
デッキ: agents/162_alakazam_v2/deck.csv
...
FEAT_GROUPS = {'optcond', 'slot_attr', 'misc', 'resource', 'selctx', 'base', 'threat2'}
...
# **キー集合は162（=104のデッキ）のまま、値だけ117のデッキに差し替える**（EXP-107）。
# deck.csv から読むと枚数0になった Shaymin(343)/Nighttime Mine(1266) のキーが消え、
# left_/pool_ の次元が黙って縮んで162の重みが別の特徴に掛かる（例外は出ない）。
...
assert sum(DECK_COUNTS.values()) == 60, sum(DECK_COUNTS.values())
_dc_from_csv = _load_deck_counts()
assert all(DECK_COUNTS[k] == _dc_from_csv.get(k, 0) for k in DECK_COUNTS), 'deck.csvと不一致'
