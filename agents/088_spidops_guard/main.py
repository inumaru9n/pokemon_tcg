"""088_spidops_guard — 075 + 対TR壁のBossバースト述語 (I-127 / EXP-088)。

ベース: 075_058_turn1 の完全コピー + _tr_boss_gate 1関数のみ追加。F088_TRBOSS=off で075と同一。

機序（CRNトレース精読で確定）: 対Spidops(083)の負け9/10でBossが手札にあるのに一度も
プレイされない。相手アクティブ=TRたね(Mewtwo/Articuno)はVeilでPH無効、巨大手札のPH火力が
死蔵され、山を吸い尽くされてサイド負け。一方で相手ベンチの進化Spidops ex(130HP)等は
無防護でPH一撃圏。→ 全条件が盤面で検証可能な時のみBossプレイを強制する加算型ゲート:
  相手場にTR Articuno / 自アクティブのPH無効 / Boss手札 / サポ未使用 /
  無防護ベンチに hand×20>=残HP の標的 → Bossプレイ+吊り先ヒント(_lines_boss_bench)
発火後のPH攻撃は既存のkills_now/MLに任せる（抑制なし・lethal層と同じ「確実な時だけ上書き」）。
...
def _ml_layer(obs: Observation) -> list[int] | None:
    ...
    cand_cls = set(cls_of)
    # EXP-088(a) 温存: TR壁対面でキル不成立の間はbossクラスを候補から除外
    # （検証可能条件付き抑制=RULE_A同型。壁が無い/キル成立なら通常運転）
    if F088_TRBOSS and "boss" in cand_cls and len(cand_cls) > 1:
        _wall, _kb = _tr_wall_state(state)
        if _wall and _kb is None:
            cand_cls = cand_cls - {"boss"}
    best_cls = max(cand_cls, key=lambda c: proba[ft.CLASS_ID[c]])
...
# ---- EXP-088: 対TR壁のBossバースト述語 (I-127) ----
# 2部構成（トレース精読の帰結）:
#  (a) 温存: TR壁対面ではBossを「今キルに繋がる時」以外プレイしない（RULE_A同型の
#      検証可能条件付き抑制。075はT3/T5に手札6枚=PH120でBossを使い切っていた）
#  (b) 強制: キル成立時（無防護ベンチに hand×20>=残HP）はBossプレイ+吊り先ヒント
F088_TRBOSS = os.environ.get("F088_TRBOSS", "on") != "off"


def _tr_wall_state(state):
    """(壁対面か, キル可能な吊り先benchインデックス or None) を返す。"""
    my = state.yourIndex
    me = state.players[my]
    op = state.players[1 - my]
    op_all = ([op.active[0]] if op.active and op.active[0] else []) \
        + [b for b in op.bench if b is not None]
    if not any(p.id == TR_Articuno for p in op_all):
        return False, None
    best = None
    hand_n = me.handCount
    for bi, p in enumerate(op.bench):
        if p is None or is_tr_basic(p.id):
            continue
        remain = p.hp
        if remain and remain > 0 and hand_n * 20 >= remain:
            if best is None or remain > best[1]:
                best = (bi, remain)
    return True, (best[0] if best else None)


def _tr_boss_gate(obs: Observation) -> list[int] | None:
    """(b)強制側: 対TR壁でキル成立時のみBossプレイを強制（加算型）。"""
    global _lines_boss_bench
    select = obs.select
    state = obs.current
    my = state.yourIndex
    me = state.players[my]
    if state.supporterPlayed:
        return None
    wall, kill_bi = _tr_wall_state(state)
    if not wall or kill_bi is None:
        return None
    op = state.players[1 - my]
    opa = op.active[0] if op.active and op.active[0] else None
    if opa is None or not is_tr_basic(opa.id):
        return None  # アクティブにPHが通るなら通常運転（吊る必要なし）
    if not any(c.id == Boss_Orders for c in (me.hand or [])):
        return None
    for i, o in enumerate(select.option):
        if o.type == OptionType.PLAY and o.area == AreaType.HAND:
            c = get_card(obs, o.area, o.index, my)
            if c is not None and c.id == Boss_Orders:
                _lines_boss_bench = kill_bi  # 既存の吊り先ヒント機構を流用
                return [i]
    return None
    ...
    if obs.select.context == SelectContext.MAIN:
        if obs.current.turn >= 2:
            ...
            if choice is not None:
                return choice
            if F088_TRBOSS:  # EXP-088: 対TR壁Bossバースト（lethal不発時のみ）
                try:
                    choice = _tr_boss_gate(obs)
                except Exception:
                    choice = None
                if choice is not None:
                    return choice
