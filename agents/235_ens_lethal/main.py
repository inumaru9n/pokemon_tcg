"""186_alakazam_v8 — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: Yushin Ito + AlphaStarmie / 156952a871 / 07-17〜08-07（170と同一データ）

**EXP-122: 語彙の基準と正規化方針を正した版**（104から board 506→458）:
- 語彙 **79枚**: 枠A 75（① ARCH_DECK 1〜2系統 ∧ ② **正例数 >= k·d_model**、k=10）
  ＋ 枠B 4（①' 3系統以上 ∧ ④効果テキストが我々を対象 ∧ ⑤有界 ∧ ⑥既存特徴と非重複）
  ②は mu でなく**正例数**で測る。1列は隠れ256ユニットへ繋がるので256本の重みを推定し、
  その1本あたりの標本数が判定量になる（mu は1枚積みの札を不当に落とす）
  Boss's Orders は `opp_used_boss` と定義上同一なので⑥で除外、Tool Scrapper は k=10 なら②を通る。**ACE SPEC は cap=1**（1枚しか入らない）
- **正規化**: 標準化は sd≈√p なので稀な事象ほど 1単位のσが発散する。
  0/1フラグ69列は**標準化をやめ**（os5_resist_vs_me は 199.7σ→1.00）、
  opp_seen 81列は **x/4**（23.7σ→0.25）、noeffect_e 6列は x/3。
  pool_/left_ は最大2.0σで健全なので**標準化のまま**（302列）
- **枠B（汎用札の脅威追跡）は入れない**。Boss's Orders は opp_used_boss と定義上同一、
  Tool Scrapper は実観測0.6-1.1%で追跡価値なし、残る3枚も寄与は測定分解能以下だった
...
model_186.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
# **詰み層のカウンタ**（スモークテストで発火を確認するため。本番でも stderr に出ない）
_LETHAL_FIRED = [0, 0]      # [発火, うちNNが既に攻撃を1位にしていた]
...
try:
    ...
    print(f"[186_alakazam_v8] class_bias.json を読めない: {_e}", file=sys.stderr)
...
if os.environ.get("NN_170", "on") != "off":
    try:
        ...
        import feat_186 as _ft
        from nn_186 import EnsemblePolicy, LSTMPolicy

        # **model_186.npz と model_186_*.npz を全て読む**（確率平均アンサンブル）。
        # 1本しか無ければ従来どおり単体。EnsemblePolicy はメンバー間の
        # 特徴キー・行動クラス・語彙・標準化統計の一致を起動時にassertする。
        _mp = os.path.join(_AGENT_DIR, "model_186.npz")
        _extra = sorted(_glob.glob(os.path.join(_AGENT_DIR, "model_186_*.npz")))
        if os.path.exists(_mp):
            _paths = [_mp] + _extra
            _NET = (EnsemblePolicy(_paths) if len(_paths) > 1 else LSTMPolicy(_mp))
            print(f"[222] モデル {len(_paths)}本 "
                  f"({'**アンサンブル**' if len(_paths) > 1 else '単体'})", file=sys.stderr)
            _FT = _ft
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_186.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[186_alakazam_v8] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[186_alakazam_v8] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[186_alakazam_v8] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[186_alakazam_v8] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
        ...
        # ==== 詰み層（EXP-141）====
        # 「この攻撃で相手アクティブをKOでき、それで最後のサイドが取り切れる」なら、
        # NNの点数によらず攻撃する。**探索を使わない確定判定**なので、本番で
        # search_begin が動かなくても機能する（CLAUDE.md の未確認事項を回避）。
        #
        # 実測（222 / 4相手×60試合）: 見逃し候補31件のうち強制攻撃で即勝ち18件
        # （真陽性率58.1%）。**見逃しがあった19試合はすべて勝っていた**ので、
        # 期待できるのは「勝ちを早く決める」ことが主で、勝敗の反転は0/19だった。
        #
        # **my_dmg_nullified で門を絞る**: ko_opp_active は打点計算だけで判定しており、
        # Mist/Rock Fighting エネや Articuno Repelling Veil による**効果の無効化を
        # 考慮していない**（EXP-138で教師が5,265回この状態で殴っていた）。
        # 無効化されている状態で強制的に殴ると、NNより明確に悪い手になる。
        if ctx_i == 0 and _NN_ACTIVE:
            try:
                _gv = dict(zip(_keys, board))
                _sel = obs.select
                if (_gv.get("lethal_for_me", 0.0) > 0.0
                        and _gv.get("ko_opp_active", 0.0) > 0.0
                        and _gv.get("my_dmg_nullified", 0.0) <= 0.0
                        and _sel.minCount <= 1 <= (_sel.maxCount or 1)):
                    _atk = [i for i in range(n_real)
                            if _sel.option[i].type == _OptionType.ATTACK]
                    if _atk:
                        _best = max(_atk, key=lambda i: sc[i])
                        _LETHAL_FIRED[0] += 1
                        if max(range(n), key=lambda i: sc[i]) in _atk:
                            _LETHAL_FIRED[1] += 1
                        _prev = [float(cids[_best]),
                                 ovecs[_best][0], ovecs[_best][1]]
                        return [_best]
            except Exception:  # noqa: BLE001  詰み層の失敗で方策を壊さない
                pass
