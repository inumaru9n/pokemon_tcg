        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    # I-046: prize-race calibration (champion prize-mapping principle).
                    # prize_diff>0 = I'm behind (more prizes left than the opponent). When
                    # behind by >=2, favor multi-prize (ex) KOs to catch up on the race —
                    # this also propagates to Boss's Orders (plan.target on a benched ex).
                    # Gated to |diff|>=2 so even games are unchanged (no mirror regression).
                    prize_diff = self.my_prizes_left - len(self.opponent.prize)
                    if prize_diff >= 2 and prize >= 2:
                        score += prize_diff * 120
