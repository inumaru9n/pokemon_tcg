"""184_alakazam_v7 — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: Yushin Ito + AlphaStarmie / 156952a871 / 07-17〜08-07（170と同一データ）

**EXP-122: 語彙の基準と正規化方針を正した版**（104から board 506→458）:
- 語彙: ① ARCH_DECK 1〜2系統 = **81枚**。**頻度閾値は置かない**
  （mu>=0.02 も 正例数>=500 も、正規化の欠陥を語彙側で補償する代理変数でしかなかった）
...
model_184.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[184_alakazam_v7] class_bias.json を読めない: {_e}", file=sys.stderr)
...
if os.environ.get("NN_170", "on") != "off":
    try:
        import feat_184 as _ft
        from nn_184 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_184.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_184.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[184_alakazam_v7] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[184_alakazam_v7] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[184_alakazam_v7] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[184_alakazam_v7] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
