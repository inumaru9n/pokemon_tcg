...
usage:
  uv run python agents/054_alakazam_lines/calib_054.py collect --dates 2026-07-09 \
      --out /tmp/dev.pkl [-w 2] [--limit 30]
  uv run python agents/054_alakazam_lines/calib_054.py fit --dev /tmp/dev.pkl \
...
AGENT_DIR = os.path.join(REPO_ROOT, "agents/054_alakazam_lines")
...
def _ensure_worker():
    ...
    for p in (REPO_ROOT, AGENT_DIR):
        if p not in sys.path:
            sys.path.insert(0, p)
    spec = importlib.util.spec_from_file_location("calib054_agent", main_path)
