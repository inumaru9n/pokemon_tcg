"""model_058.txt (lightgbm) → model_058.npz (純Python/numpy推論用) 変換 (EXP-057)。

Kaggle評価環境にlightgbmが無いため、全ツリーをフラット配列に落とす。
検証: val決定データ上で lightgbm予測との argmax一致率を報告する。

usage:
  uv run python agents/058_alakazam_policyml2/export_model_058.py [--check val.pkl]
"""
...
def export():
    ...
    bst = lgb.Booster(model_file=os.path.join(AGENT_DIR, "model_058.txt"))
    dump = bst.dump_model()
    ...
    out = os.path.join(AGENT_DIR, "model_058.npz")
    np.savez_compressed(
        out,
...
def check(bst, pkl_path, n=3000):
    ...
    z = np.load(os.path.join(AGENT_DIR, "model_058.npz"))
