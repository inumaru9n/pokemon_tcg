"""100_kangaskhan_gen — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。

教師: James Cox / James Cox & Henry Chao（チーム統合による改名。同一sig・期間連続） / sig c64b28ca9d / 07-26〜07-29（1,265試合）。プールのMega Kangaskhan枠
...
model_100.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[100_kangaskhan_gen] class_bias.json を読めない: {_e}", file=sys.stderr)

# ---- モデルと特徴のロード（失敗しても落ちない） ----
_NET = None
_FT = None
if os.environ.get("NN_100", "on") != "off":
    try:
        import feat_100 as _ft
        from nn_100 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_100.npz")
        if os.path.exists(_mp):
            ...
            if _mc is not None and len(_mc):
                ...
                print("[100_kangaskhan_gen] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[100_kangaskhan_gen] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[100_kangaskhan_gen] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[100_kangaskhan_gen] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
