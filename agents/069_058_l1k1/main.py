            ...
            if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE:
                if o.playerIndex == my_index:
                    ...
                    if card.id == Alakazam:
                        ...
                        # EXP-069/L2: Fez ex(2プライズ)昇格は本番敗因(避ける)。
                        # 他に昇格先がある限り選ばれないよう明示的に最低化
                        score = -100
                    else:
                        score += 1
                else:
...
# ---- EXP-069: 本番058の敗因矯正(L1/K1)。モデルは不変、クラス選択にゲートを被せる ----
# 本番120戦検証: 負けの50.0%が非Alakazam終局(L1)、39.1%が山切れ(K1)
F069 = os.environ.get("F069", "on") != "off"
F069_K1 = os.environ.get("F069_K1", "on") != "off"   # 山切れ抑制ゲート
F069_L1 = os.environ.get("F069_L1", "on") != "off"   # 予備Alakazam育成ゲート
_DRAW_CLASSES = {"ab_dud", "ab_fez", "evo_dud"}      # 山を消費するドロー系
_BACKUP_BUILD = {"evo_zam", "candy", "attach_basic_zam", "attach_tel_zam"}


def _l1k1_signals(state, my):
    """L1(予備Alakazam)/K1(山切れ)の状況判定。"""
    me = state.players[my]
    ...
    def _psy(p):
        return any(e.id in PSYCHIC_ENERGY_IDS for e in p.energyCards)
    ready_zams = sum(1 for p in field if p.id == Alakazam and _psy(p))
    deck = me.deckCount
    hand = me.handCount
    # K1: 山薄い+手厚い(=Dudドローで引きすぎ→山切れの局面)
    deckout_risk = deck <= 8 and hand >= 8
    # L1: 予備のready Alakazamが無い(1体以下)。終盤ほど致命的
    no_backup = ready_zams <= 1
    return deckout_risk, no_backup, ready_zams


_L1_FILLER = {"end", "pad", "poffin", "dawn", "hilda", "mine"}  # 前進しない埋め手


def _gate_class(best_cls, cls_of, proba, ft, state, my):
    """L1/K1でモデルargmaxを補正した最良クラスを返す（スケール不変の明示ルール）。"""
    if not F069:
        return best_cls
    deckout_risk, no_backup, ready_zams = _l1k1_signals(state, my)
    avail = set(cls_of)

    def best_of(pred):
        cs = [c for c in avail if pred(c)]
        return max(cs, key=lambda c: proba[ft.CLASS_ID[c]]) if cs else None

    # K1: 山切れ危険で、モデルの選択がドロー系なら → 非ドローの最良クラスに切替
    if F069_K1 and deckout_risk and best_cls in _DRAW_CLASSES:
        alt = best_of(lambda c: c not in _DRAW_CLASSES)
        if alt is not None:
            return alt

    # L1: 予備Alakazam無し & モデルが埋め手を選んだ → 2体目育成/装填に切替
    if F069_L1 and no_backup and ready_zams >= 1 and best_cls in _L1_FILLER:
        alt = best_of(lambda c: c in _BACKUP_BUILD)
        if alt is not None:
            return alt
    return best_cls
...
def _ml_layer(obs: Observation) -> list[int] | None:
    ...
    best_cls = _gate_class(
        max(set(cls_of), key=lambda c: proba[ft.CLASS_ID[c]]),
        cls_of, proba, ft, state, my)
