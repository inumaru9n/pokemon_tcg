"""EXP-089 Marnie(1ec0)選択クラス予測の学習 (I-114)。

collect_089.py の pickle から (ゾーン特徴 → 教師の行動クラス) の多クラス分類を学習。
推論は「選択可能クラスでマスクした argmax」= 実運用と同じ形で評価する。
比較基準（ベース）= 082policyの選択クラス（pb_cls）。
パラメタは train_058 踏襲（LightGBM multiclass）。

usage:
  uv run python agents/089_marnie_ml/train_089.py \
      --dev agents/089_marnie_ml/dev_089.pkl --val agents/089_marnie_ml/val_089.pkl \
      --save-best agents/089_marnie_ml/model_089
"""
