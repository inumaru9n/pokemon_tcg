"""model_089.txt (lightgbm) → model_089.npz (純Python/numpy推論用) 変換 (EXP-089)。
...
def export():
    ...
    bst = lgb.Booster(model_file=os.path.join(AGENT_DIR, "model_089.txt"))
    dump = bst.dump_model()
    ...
    out = os.path.join(AGENT_DIR, "model_089.npz")
    np.savez_compressed(
        out,
...
def check(bst, pkl_path, n=3000):
    ...
    z = np.load(os.path.join(AGENT_DIR, "model_089.npz"))
