"""EXP-089 Marnie(1ec0f47981)ダイレクト方策学習の教師データ収集 (I-114)。

教師=Luca（1ec0レシピの本家チーム。他チームは07-18以降のコピー勢で勝率が低い）の
全MAIN決定（ctx0・min=max=1・選択肢2以上）について
  - feat_089.featurize のゾーン特徴（~110次元）
  - 選択可能な行動クラスのマスク / 本人が選んだクラス
  - 082ベース方策の予測クラスとoption一致（比較基準）
を記録して pickle 保存する。列挙なし=軽量。

usage:
  uv run python agents/089_marnie_ml/collect_089.py \
      --dates 2026-07-16 2026-07-17 2026-07-18 --out dev_089.pkl -w 4
"""
...
AGENT_DIR = os.path.join(REPO_ROOT, "agents/089_marnie_ml")
...
def _ensure_worker():
    if _W.get("loaded"):
        return
    os.environ["F089_ML"] = "off"  # 収集時はML層を無効化（ベース=082ルールの素の選択）
    for p in (REPO_ROOT, AGENT_DIR, os.path.join(REPO_ROOT, "tools")):
        if p not in sys.path:
            sys.path.insert(0, p)
    spec = importlib.util.spec_from_file_location(
        "agent089c", os.path.join(AGENT_DIR, "main.py"))
...
def cmd_collect(args):
    ...
    for c, n in cnt.most_common():
        print(f"  {c:14s} {n:6d} {n/len(rows):.4f}")
    for e in errs[:3]:
        print("  ERROR", e)
...
def main():
    ...
    ap.add_argument("--team", default="Luca")
    ap.add_argument("--sig", default="1ec0f47981")
