...
Skeleton: agents/021_dragapult_sample (official Dragapult policy) with surgical
diffs only, derived from 202-replay aggregation + trace precision reads:
 - deck swap: Rare Candy/Latias/Brock/Lucky Helmet out; Dusknoir line,
   Munkidori, Moltres, Dawn in
 - goes FIRST on the coin flip (ladder 98/98; 021 chose second)
 - Phantom Dive scatter concentrates on opponent evolving bench pieces
   (bare Abra/Kadabra sniping = the structural hole 021 lacks)
 - Cursed Blast (Dusclops 50 / Dusknoir 130) snipes engine pieces, gated on
   opponent prizes >= 2; Munkidori Adrena-Brain moves 3 counters per turn
"""
...
Basic_Psychic_Energy = 5  # ×3
Basic_Dark_Energy = 7  # ×1
...
import os as _os
DISABLE_GHOSTS = _os.environ.get("X087_NOGHOST") == "1"
OLD_SCATTER = _os.environ.get("X087_OLDSCATTER") == "1"
...
def agent(obs_dict: dict) -> list[int]:
    ...
    own_damage = 0  # total damage counters value on my field (for Adrena-Brain)
    damage = 200
    for card in my_state.active:
        if card == None:
            continue
    ...
    def cursed_blast_score(amount: int) -> int:
        ...
        if DISABLE_GHOSTS:
            return -1
        if len(op_state.prize) <= 1:  # our self-KO prize would end the game
            return -1
        if my_field_total <= 2:
            return -1
        best = -1
        for p in opp_field:
            ...
            if p.hp <= amount:
                ...
                val = 15000  # chip a big ex into Phantom Dive range
            else:
                val = -1
            best = max(best, val)
        return best
    ...
    def attach_score(attach_id: int, pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id == Moltres:
            ...
            if energy_count == 0 and attach_id == Basic_Fire_Energy and not bench_attacker:
                for p in opp_field:
                    data_p = card_table[p.id]
                    if data_p.ex or data_p.megaEx:
                        return 6000
            return -1
    ...
    def hand_score(id: int, ignore_count: bool):
        ...
        if id == Dreepy:
            ...
            count = deck_counts[Dreepy] + (0 if DISABLE_GHOSTS else deck_counts[Duskull])
            if count == 0:
                score = UNNECESSARY
            else:
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            # ladder replica goes FIRST (98/98 in replays; A/B vs go-second was
            # within noise locally, so keep the behaviorally faithful choice)
            score = 1
            ...
            if card != None:
                ...
                if (context == SelectContext.SWITCH
                    ...
                    # our Cursed Blast (Dusclops 50 / Dusknoir 130) or
                    # Adrena-Brain destination (<=30): KO engine pieces first
                    ...
                    # Phantom Dive scatter (replica core): concentrate all 6 on
                    # the opponent's evolving bench pieces, lowest HP first
                    # (bare Abra -> Kadabra pattern vs Alakazam)
                    if hp > 0 and OLD_SCATTER:
            ...
            if card.id == Dreepy:
                ...
                if field_counts[Munkidori] == 0 and not DISABLE_GHOSTS:
                    score = 47000
                else:
                    score = -1
            elif card.id == Duskull:
                if duskull_line_count < 2 and not DISABLE_GHOSTS:
                    score = 43000
                else:
                    score = -1
            elif card.id == Moltres:
                score = -1  # ladder uses Moltres as a setup lead, not a bench play
            elif card.id == Budew:
                ...
                if (deck_counts[Dreepy] + deck_counts[Drakloak] + deck_counts[Dusclops]
                        + deck_counts[Dusknoir] + deck_counts[Munkidori]) > 0:
                    score = 45000
                else:
                    score = -1
            ...
            if cid == Munkidori:
                # Adrena-Brain: move up to 3 counters off our board (needs damage)
                score = -1 if DISABLE_GHOSTS else (38000 if own_damage >= 10 else -1)
            elif cid == Dusclops:
