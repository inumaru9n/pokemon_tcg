...
def _eval_search_2ply(obs: Observation, policy: LucarioPolicy) -> list[int] | None:
    ...
    # Matchup gate (EXP-008): the 2-ply override degrades the fighting mirror
    # (opponent reply gets overestimated), so disable it when the opponent's
    # board shows our own archetype's cards. Non-mirror matchups keep it.
    if policy._opponent_has({C.MAKUHITA, C.HARIYAMA, C.LUNATONE, C.SOLROCK, C.RIOLU, C.MEGA_LUCARIO_EX}):
        return None
