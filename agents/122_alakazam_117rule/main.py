"""122_alakazam_feat4 — 全選択NN方策（LSTM）の模倣学習版。tools/gen_featurizer.py が生成。
...
model_122.npz が無い場合はエンジン列挙順の先頭を返す（＝方策として無力だが落ちない）。
"""
...
try:
    ...
    print(f"[122_alakazam_feat4] class_bias.json を読めない: {_e}", file=sys.stderr)

# ---- モデルと特徴のロード（失敗しても落ちない） ----
_NET = None
_FT = None
if os.environ.get("NN_122", "on") != "off":
    try:
        import feat_122 as _ft
        from nn_122 import LSTMPolicy

        _mp = os.path.join(_AGENT_DIR, "model_122.npz")
        if os.path.exists(_mp):
            ...
            if "board_mu" in _NET.w and getattr(_NET, "b_mu", None) is None:
                raise RuntimeError(
                    "モデルに標準化統計があるのに nn_122.py が適用していない"
                    "（古い nn_*.py が同梱されている）")
            ...
            if _mc is not None and len(_mc):
                ...
                print("[122_alakazam_feat4] 警告: モデルにクラス名が記録されていないため"
        ...
        import traceback
        print(f"[122_alakazam_feat4] モデル無効化: {type(_e).__name__}: {_e}", file=sys.stderr)
...
def _wasted_attack(sel, board, keys):
    """打点が確定0の攻撃選択肢の index 集合を返す。

    `my_best_dmg` は _atk_scan が技ごとに弱点/抵抗/効果無効化を通した後の
    「実際に通る最大打点」。0 なら、どの技を撃ってもターンを渡すだけ。
    `my_var_atk` は打点が未解決の技（Dunsparce の Trading Places 等）があること
    ＝止めてよいか判断できないので触らない。
    攻撃しか選べない場合も触らない（不正選択を避ける）。
    """
    try:
        if not _NOATK0:
            return set()
        gi = keys.index
        if board[gi("my_var_atk")] > 0 or board[gi("my_best_dmg")] > 0:
            return set()
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if _tc is not None and int(obs.select.context) not in _tc:
        print(f"[122_alakazam_feat4] 未学習context {int(obs.select.context)} → この決定のみ"
    ...
    try:
        ...
        if not _BOARD_CHECKED:
            ...
            if _bad:
                print(f"[122_alakazam_feat4] {_bad} → NN無効化", file=sys.stderr)
                globals()["_NET"] = None
        ...
        if ctx_i == 0:
            _bad = _wasted_attack(obs.select, board, _FT.FEAT_KEYS)
            if _bad:
                _natk["seen"] += 1
