...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        ...
        _gv = dict(zip(_keys, board))   # ルール層の判定に使う（EXP-138）
        ...
        # ---- 無効化されている攻撃を禁じる薄いルール層（EXP-138） ----
        # **相手が Powerful Hand（ダメカン配置＝効果）を無効化する保護を持つ**とき、
        # 攻撃は打点0のまま**ターンだけを終わらせる**。本番96戦の実測:
        #   my_dmg_nullified が立った MAIN 964件(21.7%) のうち攻撃が 169件(17.5%)、
        #   **その169件の97.0%(164件)で相手HPが1も減っていない**（判定精度97%）。
        #   1試合あたり **1.7回** 手番を捨てている。教師(Yushin)も同じ欠陥を持つ
        #   （無効化状態の攻撃率 15.5% vs 通常 9.8%）ので186はそれごと模倣している。
        # EXP-137 は教師側を dw=0 にしたが、方策全体が動いて Marnie(w31.9) が
        # −0.94pt 落ち差し引き −0.25pt だった。**ルール層は発火条件の外を変えない**。
        # 代替手は方策の2位以下に任せる（我々が代替を決めない）。
        try:
            if ctx_i == 0 and float(_gv.get("my_dmg_nullified", 0.0)) > 0.0:
                ...
                # **攻撃以外に手が残るときだけ**発火する（打つ手なしでは何もしない）
                if _atk and len(_atk) < n_real:
                    # **実際に選択が変わるときだけ**発火として数える。
                    # 「無効化状態のMAIN全部」を数えると発火率を桁で誤読する。
                    if max(range(n), key=lambda i: sc[i]) in _atk:
                        _NUL_FIRED[0] += 1
                    for i in _atk:
                        sc[i] = -1e18
        except Exception:  # noqa: BLE001  ルールが失敗しても方策は生かす
            pass
