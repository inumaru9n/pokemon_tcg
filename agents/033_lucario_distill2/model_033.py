"""EXP-033: カード埋め込み方策ネット（純NumPy）。EXP-015比で容量増: D_CARD 24 / 256-128。
...
import features_033 as F

D_CARD = 24
D_ATTACK = 8
D_TOKEN = D_CARD + D_CARD + D_ATTACK + F.N_OPT_TYPE + F.N_OPT_SCALAR   # 94
D_CTX = F.N_CONTEXT + D_CARD + D_CARD + F.N_CTX_SCALAR                 # 174
D_IN = D_TOKEN + D_CTX                                                 # 268
H1, H2 = 256, 128
...
def loss_and_grads(p: dict, X, mask, idx, labels):
    B = X.shape[0]
