...
class C:
    ...
    LUMIOSE_CITY = 1267
    SHAYMIN = 343  # bench guard: prevents attack damage to non-Rule-Box benched
    LILLIES_PEARL = 1172
    LEGACY_ENERGY = 12
...
class LucarioPolicy:
    ...
    def _opponent_has_bench_snipe(self) -> bool:
        # Fezandipiti ex (Cruel Arrow 100) / Marnie's Grimmsnarl line (Shadow Bullet +30 bench)
        return self._opponent_has({140, 646, 647, 648})
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        elif pokemon.id in {C.LUNATONE, C.SHAYMIN}:
            score -= 100
    ...
    def _score_to_hand(self, card: Pokemon | Card) -> float:
        ...
        if card.id == C.MAKUHITA:
            ...
            # Fetch it only when the snipe threat is live and it isn't in play yet
            if self._opponent_has_bench_snipe() and self.field_counts[C.SHAYMIN] == 0:
                score += 55
            else:
                score -= 100
        return score
    ...
    def _score_play_pokemon(self, card: Card) -> float:
        ...
        if card.id == C.SHAYMIN:
            # Bench it only against live snipe threats; otherwise keep the slot
            # for the attacker lines (its guard doesn't cover Rule-Box Pokémon).
            if not self._opponent_has_bench_snipe():
                return -1
            score = 15000  # after core Pokémon, before trainers
        return score
